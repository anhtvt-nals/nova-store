-- Failed rotations must be eligible for a fresh replacement attempt. A failed
-- replacement can also have already removed the old sandbox, so error nodes do
-- not necessarily have a current_instance_id anymore.
...
begin
  for due_node in
    ...
    where o.status = 'active'
      ...
      and (
        j.id is null
        or (j.status = 'completed' and j.updated_at < n.next_rotation_at)
        -- run_after is the cooldown after a terminal rotation failure.
        or (j.status = 'failed' and j.run_after <= now())
      )
...
begin
  ...
  update public.proxy_provisioning_jobs set
    ...
    run_after = case
      when next_job_status = 'retry' then now() + make_interval(secs => greatest(5, retry_delay_seconds))
