-- Checkout is credit-only. Enforce the same catalogue choices server-side so
-- an edited browser request cannot bypass trial or quantity restrictions.
...
begin
  ...
  if profile_row.is_trial then
    if requested_nodes <> 1 or requested_days <> 1 then
      raise exception 'Trial accounts can rent exactly one node for one day';
    end if;
  else
    if requested_nodes <> all(array[5, 10, 20, 30]) then
      raise exception 'Node quantity must be one of: 5, 10, 20, or 30';
    end if;
  ...
  select id, name, base_price, currency, service_type into selected_product
  from public.products
  where id = target_product_id and is_active = true
  for share;
  if not found or selected_product.service_type <> 'proxy' or selected_product.base_price <= 0 then
    raise exception 'Proxy product is unavailable';
  end if;
  if selected_product.currency <> 'USD' then
    raise exception 'Credit checkout supports USD proxy products only';
  end if;
  ...
  insert into public.orders(
    ...
    'credit', total_usd, selected_product.currency, selected_product.base_price,
    requested_nodes, requested_days, selected_product.name,
    format('%s nodes', requested_nodes), now() + interval '30 minutes', credit_cost
  ...
  values (target_profile_id, -credit_cost, wallet_balance, 'order_debit',
    'order:' || new_order_id::text, format('Proxy order #%s', new_order_id));
