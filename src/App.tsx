...
function CustomerShell({ active, children }: { active: CustomerRoute; children: ReactNode }) {
  ...
  const logout = () => void signOut().then(() => { queryClient.clear(); setLocation('/'); });
  return <div className="min-h-[100dvh] bg-[#f4f8f8] text-[#142037]"><aside className={cx('fixed inset-y-0 left-0 z-40 flex w-[250px] flex-col overflow-y-auto bg-[#142037] px-4 py-5 text-white transition-transform duration-300 lg:translate-x-0', mobileOpen ? 'translate-x-0' : '-translate-x-full')}><div className="px-3"><Logo inverse /></div><div className="mt-8 px-3"><nav className="grid gap-1" aria-label="Customer portal"><Link href={items[0].href} onClick={() => setMobileOpen(false)} className={itemClass(items[0])}><Gauge size={16} className={active === 'overview' ? 'text-[#69d5d0]' : 'text-slate-300'} /><span>{items[0].label}</span></Link><p className="mono mb-1 mt-5 px-3 text-[9px] font-semibold uppercase tracking-[.2em] text-[#91e4df]">Services</p>{items.slice(1, 4).map(item => <Link key={item.key} href={item.href} onClick={() => setMobileOpen(false)} className={itemClass(item)}><item.icon size={16} className={active === item.key ? item.tone === 'blue' ? 'text-blue-300' : item.tone === 'purple' ? 'text-purple-300' : 'text-[#69d5d0]' : 'text-slate-300'} /><span>{item.label}</span></Link>)}<div className="my-4 border-t border-white/15" />{items.slice(4).map(item => <Link key={item.key} href={item.href} onClick={() => setMobileOpen(false)} className={itemClass(item)}><item.icon size={16} className={active === item.key ? 'text-[#69d5d0]' : 'text-slate-300'} /><span>{item.label}</span></Link>)}</nav></div><div className="mt-auto border-t border-white/20 px-3 pt-4"><div className="mb-3 flex items-center gap-3"><div className="grid h-8 w-8 place-items-center rounded-full bg-[#69d5d0] text-xs font-extrabold text-[#142037]">{displayName[0]?.toUpperCase() || 'C'}</div><div className="min-w-0"><p className="truncate text-[11px] font-bold text-white">{displayName}</p><p className="truncate text-[10px] text-slate-300">{user?.email}</p></div></div><div className="mb-2 flex items-center justify-between rounded-xl bg-white/10 px-3 py-2 text-[10px] font-bold text-[#91e4df]"><span>Credit</span><span>{creditBalance.isLoading ? '…' : `${(creditBalance.data?.balance || 0).toLocaleString()} cr`}</span></div><button onClick={logout} className="flex w-full items-center gap-2 rounded-xl px-3 py-2 text-[11px] font-bold text-slate-200 hover:bg-white/10 hover:text-white" aria-label={t('signOut')}><LogOut size={15} />{t('signOut')}</button></div></aside><div className="min-h-[100dvh] lg:pl-[250px]"><header className="sticky top-0 z-30 flex h-[72px] items-center justify-between border-b border-[#dbe7e9] bg-[#f4f8f8]/90 px-5 backdrop-blur lg:px-9"><button className="rounded-lg p-2 text-slate-500 lg:hidden" onClick={() => setMobileOpen(!mobileOpen)} aria-label="Open navigation"><Menu size={22} /></button><div className="hidden items-center gap-2 text-[11px] text-slate-400 sm:flex"><span className="h-1.5 w-1.5 rounded-full bg-[#69d5d0]" /> Customer workspace</div><div className="flex items-center gap-2"><LocaleSwitcher /><div className="hidden items-center gap-1.5 rounded-xl border border-[#bfe3df] bg-[#eaf8f6] px-3 py-2 sm:flex"><Zap size={14} className="text-[#13716e]" /><span className="mono text-[11px] font-extrabold text-[#13716e]">{creditBalance.isLoading ? '…' : `${(creditBalance.data?.balance || 0).toLocaleString()} cr`}</span></div>{identity.data?.role === 'admin' && <Link href="/admin" className="hidden rounded-xl px-3 py-2 text-[11px] font-bold text-[#13716e] hover:bg-[#def5f3] sm:inline-flex">{t('admin')}</Link>}<button onClick={logout} className="grid h-9 w-9 place-items-center rounded-xl border border-[#dbe7e9] bg-white text-slate-600 hover:border-[#f46c43] hover:text-[#e05c37]" aria-label={t('signOut')}><LogOut size={15} /></button></div></header>{children}</div></div>;
}
...
function DashboardBillingPage() {
  ...
  return <CustomerShell active="billing"><main className="mx-auto max-w-4xl px-5 py-9 lg:px-8 lg:py-12"><p className="mono text-[10px] uppercase tracking-[.2em] text-[#e4643d]">credit & billing</p><h1 className="mt-2 text-3xl font-extrabold tracking-[-.05em]">Billing</h1><p className="mt-2 text-sm leading-6 text-slate-500">Credit is the only checkout method. Server-side pricing and wallet validation protect every order.</p><State loading={credit.isLoading || identity.isLoading} error={credit.isError || identity.isError} onRetry={() => { void credit.refetch(); void identity.refetch(); }}><section className="mt-7 rounded-3xl bg-[#142037] p-7 text-white"><p className="mono text-[10px] uppercase tracking-[.18em] text-[#69d5d0]">Available credit</p><p className="mt-3 text-4xl font-extrabold">{(credit.data?.balance || 0).toLocaleString()} <span className="text-lg text-slate-300">cr</span></p><p className="mt-4 max-w-xl text-sm leading-6 text-slate-300">Orders for Datacenter and Residential SOCKS5 debit this same protected account balance only after server validation.</p></section>{isAdmin && <section className="mt-5 rounded-3xl border border-[#dbe7e9] bg-white p-6"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="mono text-[10px] uppercase tracking-[.16em] text-[#e4643d]">sandbox only · administrator</p><h2 className="mt-2 text-xl font-extrabold text-[#142037]">Top up Credit with Sumopod</h2><p className="mt-2 max-w-xl text-sm leading-6 text-slate-500">A server-created, signed payment invoice is required before credit can be added. The final balance changes only after Sumopod’s verified webhook.</p></div><Badge tone="orange">Sandbox</Badge></div><div className="mt-5 grid gap-3 sm:grid-cols-[1fr_.8fr_auto]"><label className="text-sm font-bold text-[#142037]">Amount (IDR)<input className="mt-2 block min-h-11 w-full rounded-xl border border-[#dbe7e9] bg-[#f8fbfb] px-3 text-sm outline-none focus:border-[#f46c43]" type="number" min="10000" max="10000000" step="1000" disabled={Boolean(payment) && invoice.data?.status === 'pending'} value={amountIdr} onChange={event => setAmountIdr(Math.max(10_000, Math.min(10_000_000, Math.trunc(Number(event.target.value) || 10_000))))} /></label><label className="text-sm font-bold text-[#142037]">Payment method<select className="mt-2 block min-h-11 w-full rounded-xl border border-[#dbe7e9] bg-[#f8fbfb] px-3 text-sm outline-none focus:border-[#f46c43]" disabled={Boolean(payment) && invoice.data?.status === 'pending'} value={method} onChange={event => setMethod(event.target.value as typeof method)}><option value="QRIS">QRIS</option><option value="QRIS_INSTANT">QRIS Instant</option></select></label><Button className="self-end min-h-11 !text-white" disabled={checkout.isPending || (Boolean(payment) && invoice.data?.status === 'pending')} onClick={startTopUp}>{checkout.isPending ? 'Creating payment…' : 'Pay with Sumopod'}</Button></div>{payment && <div className={cx('mt-4 rounded-2xl border px-4 py-3 text-sm', invoice.data?.status === 'completed' ? 'border-emerald-200 bg-emerald-50 text-emerald-800' : invoice.data?.status === 'failed' || invoice.data?.status === 'expired' ? 'border-red-200 bg-red-50 text-red-800' : 'border-[#bfe3df] bg-[#eaf8f6] text-[#13716e]')}><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="font-extrabold">{invoice.data?.status === 'completed' ? `${invoice.data.creditAmount.toLocaleString()} Credit added` : invoice.data?.status === 'failed' || invoice.data?.status === 'expired' ? `Payment ${invoice.data.status}` : 'Waiting for verified payment'}</p><p className="mt-1 text-xs">{invoice.data?.status === 'pending' || !invoice.data ? 'Open Sumopod in a new tab, complete QRIS, then keep this page open. Credit is added after the webhook is verified.' : 'The server has updated this balance from the verified Sumopod event.'}</p></div>{(invoice.data?.status === 'pending' || !invoice.data) && <a href={payment.paymentUrl} target="_blank" rel="noopener noreferrer" className="inline-flex min-h-10 items-center justify-center rounded-xl bg-[#f46c43] px-3 text-xs font-bold text-white hover:bg-[#df5934]">Open payment</a>}{invoice.data?.status !== 'pending' && <button type="button" className="text-xs font-bold underline" onClick={() => setPayment(null)}>New deposit</button>}</div></div>}<p className="mt-3 text-xs leading-5 text-slate-500">The IDR amount and Credit conversion are recalculated on the server. Sandbox checkout is intentionally visible to administrators only.</p></section>}</State></main></CustomerShell>;
}
