...
function StaticResidentialNodeCard({ node, replacementsRemaining, replacing, onReplace }: { node: StaticResidentialOrder['nodes'][number]; replacementsRemaining: number; replacing: boolean; onReplace: () => void }) {
  ...
  return <div className="min-w-0 rounded-2xl border border-[#e1eaec] bg-[#f8fbfb] p-4"><div className="flex items-center justify-between"><p className="mono text-[10px] text-slate-400">{t('port').toUpperCase()} {node.port}</p><Badge tone={node.status === 'active' ? 'green' : 'neutral'}>{node.status}</Badge></div><p className="mt-4 text-[9px] font-bold uppercase tracking-wide text-slate-400">SOCKS5</p><code className="mt-1 block break-all text-[11px] leading-5 text-[#13716e]">{socks5Value || t('unavailable')}</code>{socks5Value && <div className="mt-3 flex flex-wrap gap-3"><button onClick={() => void navigator.clipboard?.writeText(socks5Value)} className="inline-flex items-center gap-1 text-xs font-bold text-[#e05c37]"><Copy size={13} /> SOCKS5</button><button onClick={() => void navigator.clipboard?.writeText(httpValue)} className="inline-flex items-center gap-1 text-xs font-bold text-[#7c4dcc]"><Copy size={13} /> HTTP</button></div>}{httpValue && <><p className="mt-3 text-[9px] font-bold uppercase tracking-wide text-slate-400">HTTP</p><code className="mt-1 block break-all text-[11px] leading-5 text-[#7c4dcc]">{httpValue}</code></>}<div className="mt-4 border-t border-[#e1eaec] pt-3"><p className="text-[10px] leading-4 text-slate-500">{node.lastReplacedAt ? `${t('lastIpReplacement')}: ${date(node.lastReplacedAt)}` : t('fixedIpNoRotation')}</p><Button variant="outline" className="mt-3 min-h-8 w-full px-2 text-[11px]" disabled={!replacementsRemaining || replacing} onClick={onReplace}><RefreshCw size={13} />{replacing ? t('replacingIp') : `${t('replaceIp')} (${replacementsRemaining}/5)`}</Button></div></div>;
}
...
function Protected({ page }: { page: ProtectedPage }) {
  ...
  switch (page) {
    ...
    case 'client-temp-mail': return <TempMailPage />;
