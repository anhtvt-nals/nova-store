...
function DashboardBillingPage() {
  ...
  const estimatedCredit = Number(((amountIdr / usdToIdrRate) * creditsPerUsd).toFixed(2));
  ...
  return <CustomerShell active="billing"><main className="mx-auto max-w-5xl px-5 py-9 lg:px-8 lg:py-12"><p className="mono text-[10px] uppercase tracking-[.2em] text-[#e4643d]">credit & billing</p><h1 className="mt-2 text-3xl font-extrabold tracking-[-.05em]">Billing</h1><p className="mt-2 text-sm leading-6 text-slate-500">Credit is the only checkout method. Server-side pricing and wallet validation protect every order.</p><State loading={credit.isLoading || identity.isLoading} error={credit.isError || identity.isError} onRetry={() => { void credit.refetch(); void identity.refetch(); }}><div className="mt-7 grid gap-5 lg:grid-cols-[3fr_7fr]"><section className="rounded-3xl bg-[#142037] p-7 text-white"><p className="mono text-[10px] uppercase tracking-[.18em] text-[#69d5d0]">Available credit</p><p className="mt-3 text-4xl font-extrabold">{(credit.data?.balance || 0).toLocaleString()} <span className="text-lg text-slate-300">cr</span></p><p className="mt-4 max-w-xl text-sm leading-6 text-slate-300">Orders for Datacenter and Residential SOCKS5 debit this same protected account balance only after server validation.</p></section>{isAdmin && <section className="rounded-3xl border border-[#dbe7e9] bg-white p-6"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="mono text-[10px] uppercase tracking-[.16em] text-[#e4643d]">sandbox only · administrator</p><h2 className="mt-2 text-xl font-extrabold text-[#142037]">Top up Credit with Sumopod</h2><p className="mt-2 max-w-xl text-sm leading-6 text-slate-500">A server-created, signed payment invoice is required before credit can be added. The final balance changes only after Sumopod’s verified webhook.</p></div><Badge tone="orange">Sandbox</Badge></div><div className="mt-5 grid gap-3 sm:grid-cols-[1fr_.8fr_auto]"><label className="text-sm font-bold text-[#142037]">Amount (IDR)<input className="mt-2 block min-h-11 w-full rounded-xl border border-[#dbe7e9] bg-[#f8fbfb] px-3 text-sm outline-none focus:border-[#f46c43]" type="number" min="10000" max="10000000" step="1000" disabled={Boolean(payment) && invoice.data?.status === 'pending'} value={amountIdr} onChange={event => setAmountIdr(Math.max(10_000, Math.min(10_000_000, Math.trunc(Number(event.target.value) || 10_000))))} /></label><label className="text-sm font-bold text-[#142037]">Payment method<select className="mt-2 block min-h-11 w-full rounded-xl border border-[#dbe7e9] bg-[#f8fbfb] px-3 text-sm outline-none focus:border-[#f46c43]" disabled={Boolean(payment) && invoice.data?.status === 'pending'} value={method} onChange={event => setMethod(event.target.value as typeof method)}><option value="QRIS">QRIS</option><option value="QRIS_INSTANT">QRIS Instant</option></select></label><Button className="self-end min-h-11 !text-white" disabled={checkout.isPending || (Boolean(payment) && invoice.data?.status === 'pending')} onClick={startTopUp}>{checkout.isPending ? 'Creating payment…' : 'Pay with Sumopod'}</Button></div><div className="mt-3 rounded-xl border border-[#bfe3df] bg-[#eaf8f6] px-4 py-3"><p className="text-xs font-bold text-[#13716e]">Estimated Credit</p><p className="mono mt-1 text-lg font-extrabold text-[#13716e]">≈ {estimatedCredit.toLocaleString(undefined, { maximumFractionDigits: 2 })} cr</p><p className="mt-1 text-[11px] leading-4 text-[#287c79]">Reference only: {amountIdr.toLocaleString('id-ID')} IDR ÷ {usdToIdrRate.toLocaleString('id-ID')} × {creditsPerUsd.toLocaleString()} credits/USD. The server recalculates the final Credit.</p></div>{payment && <div className={cx('mt-4 rounded-2xl border px-4 py-3 text-sm', invoice.data?.status === 'completed' ? 'border-emerald-200 bg-emerald-50 text-emerald-800' : invoice.data?.status === 'failed' || invoice.data?.status === 'expired' ? 'border-red-200 bg-red-50 text-red-800' : 'border-[#bfe3df] bg-[#eaf8f6] text-[#13716e]')}><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="font-extrabold">{invoice.data?.status === 'completed' ? `${invoice.data.creditAmount.toLocaleString()} Credit added` : invoice.data?.status === 'failed' || invoice.data?.status === 'expired' ? `Payment ${invoice.data.status}` : 'Waiting for verified payment'}</p><p className="mt-1 text-xs">{invoice.data?.status === 'pending' || !invoice.data ? 'Open Sumopod in a new tab, complete QRIS, then keep this page open. Credit is added after the webhook is verified.' : 'The server has updated this balance from the verified Sumopod event.'}</p></div>{(invoice.data?.status === 'pending' || !invoice.data) && <a href={payment.paymentUrl} target="_blank" rel="noopener noreferrer" className="inline-flex min-h-10 items-center justify-center rounded-xl bg-[#f46c43] px-3 text-xs font-bold text-white hover:bg-[#df5934]">Open payment</a>}{invoice.data?.status !== 'pending' && <button type="button" className="text-xs font-bold underline" onClick={() => setPayment(null)}>New deposit</button>}</div></div>}<p className="mt-3 text-xs leading-5 text-slate-500">The IDR amount and Credit conversion are recalculated on the server. Sandbox checkout is intentionally visible to administrators only.</p></section>}</div></State></main></CustomerShell>;
}
