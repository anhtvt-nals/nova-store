...
import {
  ...
  useAddCreditTopUp, useCatalogSettings, useCreditWallets, useDeductCredit, useProxySettings, useResetUserPassword, useRevokeProviderApiKey, useUpdateCategory, useUpdateGeneralSettings, useUpdateOrderStatus, useUpdateProduct, useUpdateProvider, useUpdateProviderApiKey, useUpdateProxyPrice, useUpdateUser,
...
function ProviderApiKeyRow({ apiKey, onRefresh }: { apiKey: ProviderApiKey; onRefresh: () => void }) {
  ...
  return <div className="flex flex-wrap items-center justify-between gap-4 px-5 py-4"><div className="min-w-0"><div className="flex items-center gap-2"><p className="text-sm font-bold">{apiKey.label}</p><Badge tone={apiKey.status === 'active' ? 'green' : 'red'}>{apiKey.status}</Badge></div><p className="mt-1 text-xs text-slate-500">{apiKey.providerName}</p><p className="mono mt-1 text-[10px] text-slate-400">{apiKey.maskedKey}</p>{apiKey.status === 'revoked' && apiKey.revokedReason && <p className="mt-1 text-[11px] text-red-500">Auto-disabled: {apiKey.revokedReason}</p>}</div><div className="flex items-end gap-2"><label className="text-[10px] font-bold uppercase tracking-wide text-slate-500">Key max sandbox<input className="mt-1 block w-24 rounded-xl border border-[#dbe7e9] bg-[#f8fbfb] px-2 py-2 text-sm" type="number" min={1} step={1} value={limit} onChange={event => setLimit(event.target.value)} /></label><Button variant="outline" className="min-h-9 px-3 text-xs" disabled={!validLimit || update.isPending || parsedLimit === apiKey.maxSandboxes} onClick={() => update.mutate({ id: apiKey.id, data: { maxSandboxes: parsedLimit } }, { onSuccess: onRefresh, onError: error => window.alert(error.message) })}>{update.isPending ? 'Saving…' : 'Save'}</Button>{apiKey.status === 'active' && <Button variant="danger" className="min-h-9 px-3 text-xs" disabled={revoke.isPending} onClick={() => revoke.mutate({ id: apiKey.id }, { onSuccess: onRefresh, onError: error => window.alert(error.message) })}>Revoke</Button>}</div></div>;
}
