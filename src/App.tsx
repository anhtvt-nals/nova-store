...
function AdminProvisioningLogsPage() {
  ...
  const statusTone = (status: string) => status === 'completed' ? 'green' as const : status === 'failed' ? 'red' as const : status === 'retry' ? 'orange' as const : 'neutral' as const;
  return <PageLayout admin eyebrow="proxy operations" title="Provisioning logs" body="Admin-only job history. Errors are retained in the database and sorted by creation time, newest first."><State loading={jobs.isLoading} error={jobs.isError} onRetry={() => jobs.refetch()} empty={!jobs.data?.items.length}><div className="overflow-hidden rounded-3xl border border-[#dbe7e9] bg-white"><div className="divide-y divide-[#edf2f3]">{jobs.data?.items.map(job => <div key={job.id} className="grid gap-3 px-5 py-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><div className="flex flex-wrap items-center gap-2"><p className="font-extrabold">Job #{job.id}</p><Badge tone={statusTone(job.status)}>{job.status}</Badge><span className="mono text-[10px] uppercase tracking-wide text-slate-400">{job.action}</span></div><p className="mt-1 text-xs text-slate-500">Node #{job.nodeId}{job.orderId ? ` · Order #${job.orderId}` : ''}{job.providerName ? ` · ${job.providerName}` : ''}</p></div><div className="text-left text-xs text-slate-400 sm:text-right"><p>{new Date(job.createdAt).toLocaleString()}</p><p className="mono mt-1">attempt {job.attempts}/{job.maxAttempts}</p></div></div>{job.error ? <div className="rounded-xl border border-red-100 bg-red-50 px-3 py-2"><p className="mono break-words text-xs leading-5 text-red-700">{job.error}</p></div> : <p className="text-xs text-slate-400">No error recorded.</p>}{job.status === 'running' && job.lockedUntil && <p className="text-[11px] text-slate-500">Worker: {job.lockedBy || 'unknown'} · lease until {new Date(job.lockedUntil).toLocaleString()}</p>}{job.status === 'retry' && job.runAfter && <p className="text-[11px] text-[#b75d12]">Retry scheduled: {new Date(job.runAfter).toLocaleString()}</p>}</div>)}</div><Pagination page={page} total={jobs.data?.totalPages || 1} onChange={setPage} /></div></State></PageLayout>;
}
