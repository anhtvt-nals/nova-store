...
import {
  ...
  FolderTree, Gauge, Globe2, History, KeyRound, Layers3, Link2, LogIn, LogOut, Mail, Menu, MoreHorizontal, Network, Package, Pencil, Plus, RefreshCw, Search, UserCheck,
...
import {
  ...
  useCreateProvider, useCreateProviderApiKey, useBulkImportProviderApiKeys, useDeleteCategory, useDeleteProduct, useDeleteProvider, useDeleteSandboxKey, useDeleteUser,
  useCreditBalance, useCurrentUser, useExtendOrder, useExportProxyConnections, useExportResidentialProxyConnections, useOrderQuote, useRecreateAllProxyNodes, useRestartProxyNode, useCreateStaticResidentialOrder, useExtendStaticResidentialOrder, useReplaceStaticResidentialNode, useExportStaticResidentialConnections, useListStaticResidentialOrders, useStaticResidentialQuote, useImportStaticResidentialInventory, useCheckStaticResidentialInventoryStatus, useEnableStaticResidentialInventoryProxy, useStaticResidentialInventory, useStaticResidentialPricing, useUpdateStaticResidentialPricing, useTempMailDomains, useTempMailMessages, useCreateSumopodCheckout, useSumopodInvoice, useSumopodInvoices,
  useGetAdminOverview, useGetClientOverview, useGetOrderConnection, usePaginatedAdminOrders, usePaymentInvoices,
...
import type {
  AdminOrder, AdminProduct, CatalogProduct, Category, CreditWallet, CurrentUser, GeneralSettings, PaginatedAdminOrders, PaymentInvoice, Plan, ProductInput, ProxyNode, ProxyProvider, ProviderApiKey, RuntimeProxyNode, SandboxKey, User, Order, ConnectionDetails, StaticResidentialOrder, SumopodCheckout,
} from '@/lib/api-client';
...
function CommunityLinks({ inverse = false, className }: { inverse?: boolean; className?: string }) {
  ...
  return <div className={cx('flex flex-wrap items-center justify-center gap-x-4 gap-y-2', className)}>
    <a href={communityLinks.telegram} target="_blank" rel="noreferrer" className={cx('inline-flex items-center gap-1.5 text-xs font-bold transition', linkClass)} aria-label={t('telegramCommunity')}><Send size={14} />Telegram</a>
    <a href={communityLinks.whatsapp} target="_blank" rel="noreferrer" className={cx('inline-flex items-center gap-1.5 text-xs font-bold transition', linkClass)} aria-label={t('whatsappCommunity')}><MessageCircle size={14} />WhatsApp</a>
  </div>;
}
...
function State({ loading, error, onRetry, children, empty }: { loading?: boolean; error?: boolean; onRetry?: () => void; children: ReactNode; empty?: boolean }) {
  ...
  if (error) return <div className="rounded-2xl border border-red-100 bg-red-50 p-8 text-center"><CircleAlert className="mx-auto mb-3 text-red-500" size={24} /><p className="font-bold text-red-800">{t('loadErrorTitle')}</p><p className="mt-1 text-sm text-red-700">{t('loadErrorBody')}</p>{onRetry && <Button onClick={onRetry} variant="outline" className="mt-4"><RefreshCw size={15} /> {t('retry')}</Button>}</div>;
  if (empty) return <div className="rounded-2xl border border-dashed border-[#cbd9df] bg-white p-10 text-center"><Layers3 className="mx-auto mb-3 text-slate-300" size={28} /><p className="font-bold text-[#142037]">{t('emptyTitle')}</p><p className="mt-1 text-sm text-slate-500">{t('emptyBody')}</p></div>;
  return <>{children}</>;
}
...
function NetworkOrb() {
  ...
  return <div className="relative mx-auto aspect-square w-full max-w-[510px] [perspective:1100px]">
    ...
    <div className="absolute inset-[34%] grid place-items-center rounded-full border border-[#69d5d0]/50 bg-[#203c4b] shadow-[0_0_45px_rgba(105,213,208,.22)]">
      <div className="text-center"><div className="mono text-[10px] uppercase tracking-[.24em] text-[#69d5d0]">{t('routeCore')}</div><div className="mt-2 text-2xl font-extrabold text-white">US / 24</div><div className="mt-1 text-xs text-slate-400">{t('nodesOnline')}</div></div>
    </div>
    {[
...
function AppShell({ children, admin = false }: { children: ReactNode; admin?: boolean }) {
  ...
  const nav = admin ? [
    ...
    { section: 'Info' }, { href: '/admin/info/users', label: 'Users', icon: Users }, { href: '/admin/credits', label: 'Credits', icon: Zap }, { href: '/admin/payment-invoices', label: 'Payment invoices', icon: History },
...
function OrdersTable({ orders }: { orders: Order[] }) {
  const { formatMoney, t } = useLocalePreferences();
  return <div className="overflow-hidden rounded-3xl border border-[#dbe7e9] bg-white"><div className="hidden grid-cols-[1.3fr_1fr_.7fr_.8fr] gap-4 border-b border-[#edf2f3] px-5 py-3 text-[10px] font-bold uppercase tracking-[.15em] text-slate-400 md:grid"><span>{t('orderService')}</span><span>{t('configuration')}</span><span>{t('amount')}</span><span>{t('status')}</span></div>{orders.map(order => <div key={order.id} className="grid gap-2 border-b border-[#edf2f3] px-5 py-4 last:border-0 md:grid-cols-[1.3fr_1fr_.7fr_.8fr] md:items-center md:gap-4"><div><p className="text-[10px] font-bold uppercase tracking-[.12em] text-[#13716e]">{order.productName}</p><p className="mt-1 font-bold text-[#142037]">{order.planName}</p><p className="mono mt-1 text-[10px] text-slate-400">#{String(order.id).padStart(5, '0')} · {date(order.createdAt)}</p></div><p className="text-sm text-slate-600"><strong>{order.nodeCount}</strong> {order.nodeCount === 1 ? t('node') : t('nodes')} · <strong>{order.rentalDays}</strong> {order.rentalDays === 1 ? t('day') : t('daysLabel')}</p><p className="text-sm font-bold text-[#142037]">{formatMoney(order.amount)}</p><div><Badge tone={orderTone(order.status)}>{order.status}</Badge></div></div>)}</div>;
}
...
function UsersTable({ users, loading, onEdit, onDelete, onToggle, onResetPassword, resettingId, pagination }: { users: User[]; loading: boolean; onEdit: (user: User) => void; onDelete: (id: number) => void; onToggle: (user: User) => void; onResetPassword: (user: User) => void; resettingId?: number; pagination?: { page: number; total: number; onChange: (page: number) => void } }) {
  ...
  return <div className="overflow-hidden rounded-3xl border border-[#dbe7e9] bg-white"><State loading={loading} empty={!users.length}><div className="divide-y divide-[#edf2f3]">{displayedUsers.map(user => <div key={user.id} className="grid gap-3 px-5 py-4 sm:grid-cols-[minmax(0,1fr)_minmax(0,180px)_auto] sm:items-center" data-testid={`row-user-${user.id}`}><div className="flex min-w-0 items-center gap-3"><div className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-[#def5f3] text-xs font-extrabold text-[#13716e]">{user.name.slice(0, 1).toUpperCase()}</div><div className="min-w-0"><p className="truncate text-sm font-bold text-[#142037]" title={user.name}>{user.name}</p><p className="truncate text-xs text-slate-500" title={user.email}>{user.email}</p><p className="mt-1 truncate text-[10px] text-slate-400">Usage: {(user.usage?.today || 0).toLocaleString()} today · {(user.usage?.requests || 0).toLocaleString()} total · {user.usage?.requests ? `${Math.round(((user.usage.successful || 0) / user.usage.requests) * 100)}% success` : '—'}</p></div></div><div className="flex min-w-0 items-center gap-2"><Badge tone={user.status === 'active' ? 'green' : 'red'}>{user.status}</Badge><span className="min-w-0 truncate text-xs text-slate-500" title={user.planName}>{user.planName}</span></div><div className="flex shrink-0 items-center gap-2"><IconActionButton label="Edit user" onClick={() => onEdit(user)} testId={`button-edit-user-${user.id}`}><Pencil size={15} /></IconActionButton><IconActionButton label="Reset password" onClick={() => onResetPassword(user)} disabled={resettingId === user.id} testId={`button-reset-password-${user.id}`}><KeyRound size={15} /></IconActionButton><IconActionButton label={user.status === 'active' ? 'Suspend user' : 'Activate user'} onClick={() => onToggle(user)} testId={`button-toggle-user-${user.id}`}>{user.status === 'active' ? <Ban size={15} /> : <Power size={15} />}</IconActionButton><IconActionButton label="Delete user" tone="danger" onClick={() => onDelete(user.id)} testId={`button-delete-user-${user.id}`}><Trash2 size={15} /></IconActionButton></div></div>)}</div><Pagination page={activePage} total={pageTotal} onChange={changePage} /></State></div>;
}
...
function AdminOrdersTable({ orders: source, loading, onStatus, onCancel }: { orders: AdminOrder[] | PaginatedAdminOrders; loading: boolean; onStatus: (id: number, status: 'active' | 'rejected') => void; onCancel?: (id: number) => void }) {
    ...
    <section className="mt-10 grid gap-5 xl:grid-cols-[.72fr_1.28fr]">
      <div className="rounded-3xl border border-[#dbe7e9] bg-white p-6">
        ...
        <div className="mt-5 grid gap-3">
          ...
          <div className="grid gap-3 sm:grid-cols-2"><input className={inputClass} placeholder="Service type" value={productForm.serviceType} onChange={event => setProductForm({ ...productForm, serviceType: event.target.value })} /><select className={inputClass} value={productForm.proxyType || 'datacenter'} onChange={event => setProductForm({ ...productForm, proxyType: event.target.value as 'datacenter' | 'residential' })} disabled={productForm.serviceType !== 'proxy'}><option value="datacenter">Datacenter proxy</option><option value="residential">Residential proxy</option></select><input className={inputClass} placeholder="Country (US)" maxLength={2} value={productForm.countryCode || ''} onChange={event => setProductForm({ ...productForm, countryCode: event.target.value.toUpperCase() })} /><input className={inputClass} placeholder="Currency" maxLength={3} value={productForm.currency} onChange={event => setProductForm({ ...productForm, currency: event.target.value.toUpperCase() })} /></div>
...
function AdminOrdersPage() {
  ...
  const cancelOrder = useCancelRunningProxyOrder();
  const qc = useQueryClient();
  ...
  return <PageLayout admin eyebrow="settlement" title="Order approval queue" body="Review manual payments before provisioning customer access."><AdminOrdersTable orders={orders.data?.items || []} loading={orders.isLoading} onStatus={(id, status) => updateOrder.mutate({ id, data: { status } }, { onSuccess: refresh })} onCancel={id => { if (window.confirm(`Cancel running order #${id}? Its nodes will be terminated and cannot be restored.`)) cancelOrder.mutate({ id }, { onSuccess: refresh, onError: error => window.alert(error.message) }); }} /><Pagination page={page} total={orders.data?.totalPages || 1} onChange={setPage} /></PageLayout>;
}
...
function BulkProviderApiKeyImportDialog({ providerId, providerName, defaultMaxSandboxes, onImported, onClose }: { providerId: number; providerName: string; defaultMaxSandboxes: number; onImported: () => void; onClose: () => void }) {
  const importer = useBulkImportProviderApiKeys();
  const [content, setContent] = useState('');
  const [maxSandboxes, setMaxSandboxes] = useState(String(defaultMaxSandboxes));
  const [result, setResult] = useState<{ imported: number; failures: Array<{ line: number; message: string }> } | null>(null);
  ...
  const submit = () => {
    if (!content.trim() || !validMaxSandboxes) return;
    importer.mutate({ providerId, data: { content, maxSandboxes: parsedMaxSandboxes } }, {
      onSuccess: response => {
        setResult({ imported: response.imported.length, failures: response.failures });
        if (response.imported.length) onImported();
      },
      onError: error => window.alert(error.message),
    });
  };
  return <Modal title={`Import API keys · ${providerName}`} onClose={onClose}><p className="text-sm leading-6 text-slate-500">Paste one <code>username|token</code> value per line. Labels are generated randomly by the server and the default limit below applies to every imported key.</p><label className="mt-5 block text-sm font-bold text-[#142037]">Max sandboxes per key<input className="mt-2 block w-full rounded-xl border border-[#dbe7e9] bg-[#f8fbfb] px-3 py-3 text-sm outline-none focus:border-[#f46c43]" type="number" min={1} step={1} value={maxSandboxes} onChange={event => setMaxSandboxes(event.target.value)} /></label><label className="mt-4 block text-sm font-bold text-[#142037]">API key list<textarea className="mt-2 block min-h-48 w-full rounded-xl border border-[#dbe7e9] bg-[#f8fbfb] px-3 py-3 font-mono text-xs outline-none focus:border-[#f46c43]" autoComplete="off" spellCheck={false} placeholder={'username-01|token-01\nusername-02|token-02'} value={content} onChange={event => { setContent(event.target.value); setResult(null); }} /></label>{result && <div className={cx('mt-4 rounded-xl px-3 py-3 text-sm', result.failures.length ? 'border border-amber-200 bg-amber-50 text-amber-800' : 'border border-emerald-200 bg-emerald-50 text-emerald-800')}><p className="font-bold">Imported {result.imported} key{result.imported === 1 ? '' : 's'}.</p>{result.failures.length > 0 && <ul className="mt-2 list-disc space-y-1 pl-5 text-xs">{result.failures.map(failure => <li key={`${failure.line}-${failure.message}`}>Line {failure.line}: {failure.message}</li>)}</ul>}</div>}<div className="mt-6 flex gap-3"><Button variant="outline" className="flex-1" onClick={onClose}>Close</Button><Button className="flex-1" disabled={!content.trim() || !validMaxSandboxes || importer.isPending} onClick={submit}>{importer.isPending ? 'Importing…' : 'Import keys'}</Button></div></Modal>;
}
...
function AdminProviderApiKeysPage() {
  ...
  const inputClass = 'rounded-xl border border-[#dbe7e9] bg-[#f8fbfb] px-3 py-3 text-sm outline-none focus:border-[#f46c43]';
  return <><PageLayout admin eyebrow="proxy module" title="Provider API keys" body="Set a safety limit per API key. Provider Max sandbox remains the aggregate hard cap for the provider."><div className="grid gap-5 xl:grid-cols-[.7fr_1.3fr]"><div className="rounded-3xl border border-[#dbe7e9] bg-white p-6"><div className="flex items-center justify-between gap-3"><h2 className="font-extrabold">Add provider key</h2><Button variant="outline" className="min-h-9 px-3 text-xs" disabled={!selectedProvider} onClick={() => setBulkImportOpen(true)}><Download size={14} /> Import</Button></div><div className="mt-5 grid gap-3"><select className={inputClass} value={providerId || ''} onChange={event => setProviderId(Number(event.target.value))}><option value="">Select provider</option>{providers.data?.map(provider => <option key={provider.id} value={provider.id}>{provider.name}</option>)}</select><input className={inputClass} placeholder="Label" value={label} onChange={event => setLabel(event.target.value)} /><input className={inputClass} type="password" autoComplete="new-password" placeholder={isGithub ? 'GITHUB_OWNER|GITHUB_API_KEY' : 'Provider API secret'} value={secret} onChange={event => setSecret(event.target.value)} /><label className="text-xs font-bold text-slate-600">Max sandboxes for this key<input className={`${inputClass} mt-2 w-full`} type="number" min={1} step={1} value={maxSandboxes} onChange={event => setMaxSandboxes(event.target.value)} /></label>{isGithub && <p className="text-xs leading-5 text-slate-500">Format: <code>GITHUB_OWNER|GITHUB_API_KEY</code>. The owner is retained only as a masked prefix; the complete value is encrypted at rest.</p>}<p className="text-xs leading-5 text-slate-500">Requires `PROVIDER_SECRET_ENCRYPTION_KEY` on the Nest server.</p><Button disabled={!providerId || !label || secret.length < 8 || !validMaxSandboxes || createKey.isPending} onClick={submit}>{createKey.isPending ? 'Encrypting…' : 'Save encrypted key'}</Button></div></div><State loading={keys.isLoading} error={keys.isError} onRetry={() => keys.refetch()} empty={!keys.data?.length}><div className="overflow-hidden rounded-3xl border border-[#dbe7e9] bg-white"><div className="divide-y divide-[#edf2f3]">{visibleKeys.map(key => <ProviderApiKeyRow key={key.id} apiKey={key} onRefresh={refresh} />)}</div><Pagination page={page} total={totalPages} onChange={setPage} /></div></State></div></PageLayout>{bulkImportOpen && selectedProvider && <BulkProviderApiKeyImportDialog providerId={selectedProvider.id} providerName={selectedProvider.name} defaultMaxSandboxes={validMaxSandboxes ? parsedMaxSandboxes : 10} onImported={refresh} onClose={() => setBulkImportOpen(false)} />}</>;
}
...
function paymentInvoiceTone(status: PaymentInvoice['status']): 'green' | 'orange' | 'red' | 'neutral' {
  return status === 'completed' ? 'green' : status === 'pending' ? 'orange' : status === 'failed' ? 'red' : 'neutral';
}

function AdminPaymentInvoicesPage() {
  const [page, setPage] = useState(1);
  const invoices = usePaymentInvoices(page, 20);
  useEffect(() => { if (invoices.data && page > invoices.data.totalPages) setPage(invoices.data.totalPages); }, [invoices.data?.totalPages, page]);
  return <PageLayout admin eyebrow="payments" title="Payment invoices" body="Server-created Sumopod top-up invoices. Credit is granted only after a verified payment.completed webhook."><State loading={invoices.isLoading} error={invoices.isError} onRetry={() => invoices.refetch()} empty={!invoices.data?.items.length}><div className="overflow-hidden rounded-3xl border border-[#dbe7e9] bg-white"><div className="hidden grid-cols-[1.25fr_.9fr_.85fr_.85fr] gap-4 border-b border-[#edf2f3] px-5 py-3 text-[10px] font-bold uppercase tracking-[.15em] text-slate-400 lg:grid"><span>Customer / invoice</span><span>Payment</span><span>Settlement</span><span>Status</span></div><div className="divide-y divide-[#edf2f3]">{invoices.data?.items.map(invoice => <div key={invoice.id} className="grid gap-3 px-5 py-4 lg:grid-cols-[1.25fr_.9fr_.85fr_.85fr] lg:items-start lg:gap-4"><div className="min-w-0"><p className="truncate text-sm font-extrabold text-[#142037]">{invoice.customerName || invoice.customerEmail || 'Unknown customer'}</p><p className="truncate text-xs text-slate-500">{invoice.customerEmail || '—'}</p><p className="mono mt-2 break-all text-[10px] text-slate-400">{invoice.merchantOrderId}</p><p className="mt-1 text-[11px] text-slate-400">Created {new Date(invoice.createdAt).toLocaleString()}</p></div><div className="min-w-0"><p className="text-xs font-bold text-[#142037]">{invoice.providerPaymentId ? 'Sumopod payment' : 'Awaiting payment'}</p><p className="mono mt-1 break-all text-[10px] text-slate-400">{invoice.providerPaymentId || '—'}</p><p className="mt-2 text-[11px] text-slate-500">Expires {new Date(invoice.expiresAt).toLocaleString()}</p></div><div><p className="text-sm font-extrabold text-[#142037]">{invoice.amountIdr.toLocaleString('id-ID')} IDR</p><p className="mt-1 text-xs text-slate-500">Fee: {invoice.feeIdr === null ? '—' : `${invoice.feeIdr.toLocaleString('id-ID')} IDR`}</p><p className="mt-1 text-xs text-[#13716e]">Credit: {invoice.creditAmount.toLocaleString(undefined, { maximumFractionDigits: 2 })} cr</p></div><div><Badge tone={paymentInvoiceTone(invoice.status)}>{invoice.status}</Badge><p className="mt-2 text-[11px] text-slate-500">{invoice.completedAt ? `Completed ${new Date(invoice.completedAt).toLocaleString()}` : `Updated ${new Date(invoice.updatedAt).toLocaleString()}`}</p></div></div>)}</div><Pagination page={page} total={invoices.data?.totalPages || 1} onChange={setPage} /></div></State></PageLayout>;
}

function ProxyPriceRow({ setting }: { setting: { id: number; name: string; proxyType: 'datacenter' | 'residential'; countryCode: string | null; basePrice: number; currency: string; isActive: boolean } }) {
...
function AdminStaticResidentialPage() {
  ...
  const checkInventory = () => inventoryChecker.mutate(undefined, { onSuccess: result => { toast({ title: 'Proxy health check complete', description: `${result.healthy}/${result.checked} healthy · ${result.failed} failed · ${result.disabled} disabled after ${result.failureThreshold} failures · ${result.nodesSuspended} customer node${result.nodesSuspended === 1 ? '' : 's'} suspended for replacement.` }); void qc.invalidateQueries({ queryKey: getStaticResidentialInventoryQueryKey() }); }, onError: error => toast({ variant: 'destructive', title: 'Proxy health check failed', description: error.message }) });
...
function ClientDockNav({ active }: { active: 'services' | 'proxy' | 'static' }) {
  const { t } = useLocalePreferences();
  const items = [
    { href: '/client', label: t('services'), icon: Layers3, active: active === 'services' },
    { href: '/client/proxy', label: t('clientNavProxy'), icon: Network, active: active === 'proxy' },
    { href: '/client/static-residential', label: t('clientNavStatic'), icon: Globe2, active: active === 'static' },
  ];
...
function ClientHeader({ active = 'services' }: { active?: 'services' | 'proxy' | 'static' }) {
  ...
  return <><ClientDockNav active={active} /><header className="sticky top-0 z-40 border-b border-[#dbe7e9] bg-white/90 backdrop-blur-xl"><div className="mx-auto flex h-[72px] max-w-7xl items-center justify-between gap-4 px-5 lg:px-8"><Logo /><div className="flex items-center gap-2">{identity.data?.isTrial && <span className="inline-flex rounded-full border border-amber-200 bg-amber-50 px-2 py-1 text-[9px] font-extrabold uppercase tracking-wide text-amber-700">{t('trialStatus')}</span>}<LocaleSwitcher /><Link href="/dashboard/billing" className="hidden items-center gap-1.5 rounded-xl border border-[#bfe3df] bg-[#eaf8f6] px-3 py-2 transition hover:border-[#69d5d0] hover:bg-[#def5f3] sm:flex" title={t('creditBalance')} aria-label={t('creditBalance')}><Zap size={14} className="text-[#13716e]" /><span className="mono text-xs font-extrabold text-[#13716e]">{creditBalance.isLoading ? '…' : `${(creditBalance.data?.balance || 0).toLocaleString()} cr`}</span></Link>{identity.data?.role === 'admin' && <Link href="/admin" className="hidden rounded-xl px-3 py-2 text-xs font-bold text-[#13716e] hover:bg-[#def5f3] sm:inline-flex">{t('admin')}</Link>}<div className="hidden text-right sm:block"><p className="text-xs font-bold">{displayName}</p><p className="text-[10px] text-slate-500">{user?.email}</p></div><button onClick={logout} className="grid h-10 w-10 place-items-center rounded-xl border border-[#dbe7e9] bg-white text-slate-600 hover:border-[#f46c43] hover:text-[#e05c37]" aria-label={t('signOut')}><LogOut size={16} /></button></div></div></header></>;
}
...
type CustomerRoute = 'overview' | 'datacenter' | 'residential' | 'static' | 'temp-mail' | 'orders' | 'nodes' | 'billing';
...
function CustomerShell({ active, children }: { active: CustomerRoute; children: ReactNode }) {
  ...
  const items = [
    ...
    { key: 'static' as const, href: '/dashboard/static-residential', label: t('clientStaticService'), icon: Network, tone: 'neutral' },
    { key: 'temp-mail' as const, href: '/dashboard/temp-mail', label: 'Temp Mail', icon: Mail, tone: 'neutral' },
  ...
  const itemClass = (item: typeof items[number]) => cx('flex items-center gap-3 rounded-xl px-3 py-2.5 text-[11px] font-semibold transition', active === item.key ? item.tone === 'blue' ? 'bg-blue-500/15 text-white shadow-[inset_3px_0_0_#60a5fa]' : item.tone === 'purple' ? 'bg-purple-500/15 text-white shadow-[inset_3px_0_0_#c084fc]' : 'bg-white/12 text-white shadow-[inset_3px_0_0_#69d5d0]' : 'text-slate-300 hover:bg-white/8 hover:text-white');
  const logout = () => void signOut().then(() => { queryClient.clear(); setLocation('/'); });
  return <div className="min-h-[100dvh] bg-[#f4f8f8] text-[#142037]"><aside className={cx('fixed inset-y-0 left-0 z-40 flex w-[250px] flex-col overflow-y-auto bg-[#142037] px-4 py-5 text-white transition-transform duration-300 lg:translate-x-0', mobileOpen ? 'translate-x-0' : '-translate-x-full')}><div className="px-3"><Logo inverse /></div><div className="mt-8 px-3"><nav className="grid gap-1" aria-label="Customer portal"><Link href={items[0].href} onClick={() => setMobileOpen(false)} className={itemClass(items[0])}><Gauge size={16} className={active === 'overview' ? 'text-[#69d5d0]' : 'text-slate-300'} /><span>{items[0].label}</span></Link><p className="mono mb-1 mt-5 px-3 text-[9px] font-semibold uppercase tracking-[.2em] text-[#91e4df]">Services</p>{items.slice(1, 4).map(item => <Link key={item.key} href={item.href} onClick={() => setMobileOpen(false)} className={itemClass(item)}><item.icon size={16} className={active === item.key ? item.tone === 'blue' ? 'text-blue-300' : item.tone === 'purple' ? 'text-purple-300' : 'text-[#69d5d0]' : 'text-slate-300'} /><span>{item.label}</span></Link>)}<div className="my-4 border-t border-white/15" />{items.slice(4).map(item => <Link key={item.key} href={item.href} onClick={() => setMobileOpen(false)} className={itemClass(item)}><item.icon size={16} className={active === item.key ? 'text-[#69d5d0]' : 'text-slate-300'} /><span>{item.label}</span></Link>)}</nav></div><div className="mt-auto border-t border-white/20 px-3 pt-4"><div className="mb-3 flex items-center gap-3"><div className="grid h-8 w-8 place-items-center rounded-full bg-[#69d5d0] text-xs font-extrabold text-[#142037]">{displayName[0]?.toUpperCase() || 'C'}</div><div className="min-w-0"><p className="truncate text-[11px] font-bold text-white">{displayName}</p><p className="truncate text-[10px] text-slate-300">{user?.email}</p></div></div><Link href="/dashboard/billing" onClick={() => setMobileOpen(false)} className="mb-2 flex items-center justify-between rounded-xl bg-white/10 px-3 py-2 text-[10px] font-bold text-[#91e4df] transition hover:bg-white/15" title={t('creditBalance')}><span>Credit</span><span>{creditBalance.isLoading ? '…' : `${(creditBalance.data?.balance || 0).toLocaleString()} cr`}</span></Link><button onClick={logout} className="flex w-full items-center gap-2 rounded-xl px-3 py-2 text-[11px] font-bold text-slate-200 hover:bg-white/10 hover:text-white" aria-label={t('signOut')}><LogOut size={15} />{t('signOut')}</button></div></aside><div className="min-h-[100dvh] lg:pl-[250px]"><header className="sticky top-0 z-30 flex h-[72px] items-center justify-between border-b border-[#dbe7e9] bg-[#f4f8f8]/90 px-5 backdrop-blur lg:px-9"><button className="rounded-lg p-2 text-slate-500 lg:hidden" onClick={() => setMobileOpen(!mobileOpen)} aria-label="Open navigation"><Menu size={22} /></button><div className="hidden items-center gap-2 text-[11px] text-slate-400 sm:flex"><span className="h-1.5 w-1.5 rounded-full bg-[#69d5d0]" /> Customer workspace</div><div className="flex items-center gap-2">{identity.data?.isTrial && <span className="inline-flex rounded-full border border-amber-200 bg-amber-50 px-2 py-1 text-[9px] font-extrabold uppercase tracking-wide text-amber-700">{t('trialStatus')}</span>}<LocaleSwitcher /><Link href="/dashboard/billing" className="hidden items-center gap-1.5 rounded-xl border border-[#bfe3df] bg-[#eaf8f6] px-3 py-2 transition hover:border-[#69d5d0] hover:bg-[#def5f3] sm:flex" title={t('creditBalance')} aria-label={t('creditBalance')}><Zap size={14} className="text-[#13716e]" /><span className="mono text-[11px] font-extrabold text-[#13716e]">{creditBalance.isLoading ? '…' : `${(creditBalance.data?.balance || 0).toLocaleString()} cr`}</span></Link>{identity.data?.role === 'admin' && <Link href="/admin" className="hidden rounded-xl px-3 py-2 text-[11px] font-bold text-[#13716e] hover:bg-[#def5f3] sm:inline-flex">{t('admin')}</Link>}<button onClick={logout} className="grid h-9 w-9 place-items-center rounded-xl border border-[#dbe7e9] bg-white text-slate-600 hover:border-[#f46c43] hover:text-[#e05c37]" aria-label={t('signOut')}><LogOut size={15} /></button></div></header>{children}</div></div>;
}

function ClientDashboardPage() {
  const overview = useGetClientOverview();
  const { t } = useLocalePreferences();
  return <CustomerShell active="overview"><main className="mx-auto max-w-7xl px-5 py-9 lg:px-8 lg:py-12"><p className="mono text-[10px] uppercase tracking-[.2em] text-[#e4643d]">{t('dashboardEyebrow')}</p><h1 className="mt-2 text-3xl font-extrabold tracking-[-.05em] md:text-4xl">{t('dashboardOverview')}</h1><p className="mt-2 max-w-xl text-sm leading-6 text-slate-500">{t('dashboardOverviewBody')}</p><State loading={overview.isLoading} error={overview.isError} onRetry={() => overview.refetch()}><section className="mt-7 grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><Metric label={t('activeNodes')} value={String(overview.data?.activeNodes ?? 0)} detail={t('dashboardAcrossServices')} icon={Server} tone="teal" /><Metric label={t('requestsToday')} value={(overview.data?.requestsToday ?? 0).toLocaleString()} detail={t('proxyTraffic')} icon={Activity} tone="orange" /><Metric label={t('totalBandwidth')} value={bytes(overview.data?.totalBandwidthBytes ?? 0)} detail={t('uploadDownload')} icon={Gauge} tone="teal" /><Metric label={t('successRate')} value={`${overview.data?.successRate ?? 0}%`} detail={t('last24Hours')} icon={Signal} tone="orange" /></section><section className="mt-7 grid gap-4 md:grid-cols-2"><Link href="/dashboard/datacenter" className="rounded-3xl border-t-4 border-blue-500 bg-white p-6 shadow-[0_10px_35px_rgba(20,32,55,.04)]"><Badge tone="neutral">{t('datacenterSocks')}</Badge><h2 className="mt-4 text-xl font-extrabold">{t('dashboardDatacenterTitle')}</h2><p className="mt-2 text-sm leading-6 text-slate-500">{t('dashboardDatacenterBody')}</p><span className="mt-5 inline-flex items-center gap-1 text-sm font-bold text-blue-700">{t('dashboardOpenDatacenter')} <ArrowRight size={15} /></span></Link><Link href="/dashboard/residential" className="rounded-3xl border-t-4 border-purple-500 bg-white p-6 shadow-[0_10px_35px_rgba(20,32,55,.04)]"><Badge tone="neutral">{t('residentialSocks')}</Badge><h2 className="mt-4 text-xl font-extrabold">{t('dashboardResidentialTitle')}</h2><p className="mt-2 text-sm leading-6 text-slate-500">{t('dashboardResidentialBody')}</p><span className="mt-5 inline-flex items-center gap-1 text-sm font-bold text-purple-700">{t('dashboardOpenResidential')} <ArrowRight size={15} /></span></Link></section></State></main></CustomerShell>;
}
...
function DashboardOrdersPage() {
  ...
  return <CustomerShell active="orders"><main className="mx-auto max-w-7xl px-5 py-9 lg:px-8 lg:py-12"><p className="mono text-[10px] uppercase tracking-[.2em] text-[#e4643d]">{t('ordersEyebrow')}</p><h1 className="mt-2 text-3xl font-extrabold tracking-[-.05em]">{t('dashboardOrders')}</h1><p className="mt-2 text-sm text-slate-500">{t('dashboardOrdersBody')}</p><div className="mt-6 flex flex-wrap gap-3"><select aria-label={t('filterService')} className="rounded-xl border border-[#dbe7e9] bg-white px-3 py-2 text-sm" value={service} onChange={event => setService(event.target.value as typeof service)}><option value="all">{t('allServices')}</option><option value="datacenter">{t('datacenterSocks')}</option><option value="residential">{t('residentialSocks')}</option></select><select aria-label={t('filterOrderStatus')} className="rounded-xl border border-[#dbe7e9] bg-white px-3 py-2 text-sm" value={status} onChange={event => setStatus(event.target.value as typeof status)}><option value="all">{t('allStatuses')}</option>{Array.from(new Set((orders.data || []).map(order => order.status))).map(value => <option key={value} value={value}>{value.replace('_', ' ')}</option>)}</select></div><section className="mt-5"><State loading={orders.isLoading} error={orders.isError} onRetry={() => orders.refetch()} empty={!filtered.length}><OrdersTable orders={filtered} /></State></section></main></CustomerShell>;
}
...
function DashboardNodesPage() {
  ...
  return <CustomerShell active="nodes"><main className="mx-auto max-w-7xl px-5 py-9 lg:px-8 lg:py-12"><p className="mono text-[10px] uppercase tracking-[.2em] text-[#e4643d]">{t('nodesEyebrow')}</p><h1 className="mt-2 text-3xl font-extrabold tracking-[-.05em]">{t('dashboardNodes')}</h1><p className="mt-2 text-sm text-slate-500">{t('dashboardNodesBody')}</p><select aria-label={t('filterNodeService')} className="mt-6 rounded-xl border border-[#dbe7e9] bg-white px-3 py-2 text-sm" value={service} onChange={event => setService(event.target.value as typeof service)}><option value="all">{t('allServices')}</option><option value="datacenter">{t('datacenterSocks')}</option><option value="residential">{t('residentialSocks')}</option></select><section className="mt-5"><State loading={orders.isLoading || nodes.isLoading} error={orders.isError || nodes.isError} onRetry={() => { void orders.refetch(); void nodes.refetch(); }} empty={!filtered.length}><PagedProxyNodeGrid nodes={filtered} orderById={orderById} /></State></section></main></CustomerShell>;
}

function BillingInvoiceHistory() {
  const [page, setPage] = useState(1);
  const invoices = useSumopodInvoices(page, 10);
  useEffect(() => { if (invoices.data && page > invoices.data.totalPages) setPage(invoices.data.totalPages); }, [invoices.data?.totalPages, page]);
  return <section className="mt-8"><SectionTitle eyebrow="payment history" title="Deposit invoices" /><State loading={invoices.isLoading} error={invoices.isError} onRetry={() => invoices.refetch()} empty={!invoices.data?.items.length}><div className="overflow-hidden rounded-3xl border border-[#dbe7e9] bg-white"><div className="divide-y divide-[#edf2f3]">{invoices.data?.items.map(invoice => <div key={invoice.id} className="grid gap-3 px-5 py-4 sm:grid-cols-[1fr_auto] sm:items-center"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><p className="text-sm font-extrabold text-[#142037]">{invoice.amountIdr.toLocaleString('id-ID')} IDR</p><Badge tone={invoice.status === 'completed' ? 'green' : invoice.status === 'pending' ? 'orange' : invoice.status === 'failed' ? 'red' : 'neutral'}>{invoice.status}</Badge></div><p className="mt-1 text-xs text-slate-500">Fee: {invoice.feeIdr === null ? '—' : `${invoice.feeIdr.toLocaleString('id-ID')} IDR`} · Credit: {invoice.creditAmount.toLocaleString(undefined, { maximumFractionDigits: 2 })} cr</p><p className="mono mt-1 text-[10px] text-slate-400">{invoice.id}</p></div><div className="text-left text-xs text-slate-400 sm:text-right"><p>{new Date(invoice.createdAt).toLocaleString()}</p>{invoice.completedAt && <p className="mt-1 text-[#13716e]">Completed {new Date(invoice.completedAt).toLocaleString()}</p>}</div></div>)}</div><Pagination page={page} total={invoices.data?.totalPages || 1} onChange={setPage} /></div></State></section>;
}

function DashboardBillingPage() {
  const { usdToIdrRate, creditsPerUsd, t } = useLocalePreferences();
  const queryClient = useQueryClient();
  const credit = useCreditBalance();
  const identity = useCurrentUser();
  const checkout = useCreateSumopodCheckout();
  const [amountIdr, setAmountIdr] = useState<number | ''>(10_000);
  ...
  // Sumopod's signed net_amount. The active live QRIS method is 0.7% + Rp300.
  const depositAmountIdr = typeof amountIdr === 'number' && Number.isSafeInteger(amountIdr) && amountIdr >= 10_000 ? amountIdr : 10_000;
  const gatewayFeeRate = 0.007;
  const estimatedFeeIdr = Math.round(depositAmountIdr * gatewayFeeRate) + 300;
  const estimatedCredit = Number(((depositAmountIdr / usdToIdrRate) * creditsPerUsd).toFixed(2));
  ...
  const updateDepositAmount = (rawValue: string) => {
    if (rawValue.trim() === '') { setAmountIdr(''); return; }
  ...
  useEffect(() => {
    const normalizeOnBlur = (event: FocusEvent) => {
      ...
      // This selector is intentionally specific to the Sumopod deposit input
      // until the legacy one-line billing JSX is split into subcomponents.
      if (!(target instanceof HTMLInputElement) || !target.matches('input[type="number"][min="10000"][step="1000"]')) return;
      const parsed = Number(target.value);
      if (!Number.isSafeInteger(parsed) || parsed < 10_000) setAmountIdr(10_000);
    };
    window.addEventListener('focusout', normalizeOnBlur, true);
    return () => window.removeEventListener('focusout', normalizeOnBlur, true);
  }, []);
  const startTopUp = () => checkout.mutate({ data: { amountIdr: depositAmountIdr, paymentMethod: method } }, {
    onSuccess: created => { setPayment(created); },
    onError: error => window.alert(error.message),
  });
  useEffect(() => {
    if (invoice.data?.status !== 'completed') return;
    void credit.refetch();
    void queryClient.invalidateQueries({ queryKey: getCurrentUserQueryKey() });
  }, [invoice.data?.status, credit, queryClient]);
  ...
  const feeFormula = `${(gatewayFeeRate * 100).toFixed(1)}% + 300 IDR`;
  return <CustomerShell active="billing"><main className="mx-auto max-w-5xl px-5 py-9 lg:px-8 lg:py-12"><p className="mono text-[10px] uppercase tracking-[.2em] text-[#e4643d]">{t('billingEyebrow')}</p><h1 className="mt-2 text-3xl font-extrabold tracking-[-.05em]">{t('billingTitle')}</h1><p className="mt-2 text-sm leading-6 text-slate-500">{t('billingBody')}</p><State loading={credit.isLoading || identity.isLoading} error={credit.isError || identity.isError} onRetry={() => { void credit.refetch(); void identity.refetch(); }}><div className="mt-7 grid gap-5 lg:grid-cols-[3fr_7fr]"><section className="rounded-3xl bg-[#142037] p-7 text-white"><p className="mono text-[10px] uppercase tracking-[.18em] text-[#69d5d0]">{t('availableCredit')}</p><p className="mt-3 text-4xl font-extrabold">{(credit.data?.balance || 0).toLocaleString()} <span className="text-lg text-slate-300">cr</span></p><p className="mt-4 max-w-xl text-sm leading-6 text-slate-300">{t('billingBalanceBody')}</p></section>{isAdmin && <section className="rounded-3xl border border-[#dbe7e9] bg-white p-6"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="mono text-[10px] uppercase tracking-[.16em] text-[#e4643d]">{t('securePayment')}</p><h2 className="mt-2 text-xl font-extrabold text-[#142037]">{t('topupCredit')}</h2>{t('topupCreditBody') && <p className="mt-2 max-w-xl text-sm leading-6 text-slate-500">{t('topupCreditBody')}</p>}</div><Badge tone="orange">{t('livePayment')}</Badge></div><div className="mt-5 grid gap-3 sm:grid-cols-[1fr_.8fr_auto]"><label className="text-sm font-bold text-[#142037]">{t('amountYouPay')}<input className="mt-2 block min-h-11 w-full rounded-xl border border-[#dbe7e9] bg-[#f8fbfb] px-3 text-sm outline-none focus:border-[#f46c43]" type="number" min="10000" step="1000" disabled={Boolean(payment) && invoice.data?.status === 'pending'} value={amountIdr} onChange={event => updateDepositAmount(event.target.value)} /></label><label className="text-sm font-bold text-[#142037]">{t('paymentMethod')}<select className="mt-2 block min-h-11 w-full rounded-xl border border-[#dbe7e9] bg-[#f8fbfb] px-3 text-sm outline-none focus:border-[#f46c43]" disabled={Boolean(payment) && invoice.data?.status === 'pending'} value={method} onChange={event => setMethod(event.target.value as typeof method)}><option value="qris">QRIS</option></select></label><Button className="self-end min-h-11 !text-white" disabled={checkout.isPending || (Boolean(payment) && invoice.data?.status === 'pending')} onClick={startTopUp}>{checkout.isPending ? t('creatingPayment') : t('payWithSumopod')}</Button></div><p className="mt-2 text-[11px] font-medium text-slate-500">{t('minimumDeposit')}</p><div className="mt-3 rounded-xl border border-[#bfe3df] bg-[#eaf8f6] px-4 py-3"><p className="text-xs font-bold text-[#13716e]">{t('paymentCreditEstimate')}</p><div className="mt-2 grid gap-1 text-xs text-[#287c79] sm:grid-cols-3"><span>{t('youPay')}: <b>{depositAmountIdr.toLocaleString('id-ID')} IDR</b></span><span>{t('gatewayFee')}: <b>{settledFeeIdr.toLocaleString('id-ID')} IDR</b></span></div><p className="mono mt-2 text-lg font-extrabold text-[#13716e]">≈ {settledCredit.toLocaleString(undefined, { maximumFractionDigits: 2 })} cr</p><p className="mt-1 text-[11px] leading-4 text-[#287c79]">{t('paymentFeeEstimate').replace('{fee}', feeFormula)}</p></div>{payment && <div className={cx('mt-4 rounded-2xl border px-4 py-3 text-sm', invoice.data?.status === 'completed' ? 'border-emerald-200 bg-emerald-50 text-emerald-800' : invoice.data?.status === 'failed' || invoice.data?.status === 'expired' ? 'border-red-200 bg-red-50 text-red-800' : 'border-[#bfe3df] bg-[#eaf8f6] text-[#13716e]')}><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="font-extrabold">{invoice.data?.status === 'completed' ? `${invoice.data.creditAmount.toLocaleString()} ${t('creditAdded')}` : invoice.data?.status === 'failed' || invoice.data?.status === 'expired' ? `${t('paymentStatusPrefix')} ${invoice.data.status}` : t('waitingVerifiedPayment')}</p><p className="mt-1 text-xs">{invoice.data?.status === 'pending' || !invoice.data ? t('paymentPendingBody') : t('paymentSettledBody')}</p></div>{(invoice.data?.status === 'pending' || !invoice.data) && <a href={payment.paymentUrl} target="_blank" rel="noopener noreferrer" className="inline-flex min-h-10 items-center justify-center rounded-xl bg-[#f46c43] px-3 text-xs font-bold text-white hover:bg-[#df5934]">{t('openPayment')}</a>}{invoice.data?.status !== 'pending' && <button type="button" className="text-xs font-bold underline" onClick={() => setPayment(null)}>{t('newDeposit')}</button>}</div></div>}{t('billingFooter') && <p className="mt-3 text-xs leading-5 text-slate-500">{t('billingFooter')}</p>}</section>}</div></State><BillingInvoiceHistory /></main></CustomerShell>;
}

function ProxyCheckoutGuide({ proxyType }: { proxyType: 'datacenter' | 'residential' }) {
  const { t } = useLocalePreferences();
  const accent = proxyType === 'residential' ? 'border-purple-200 bg-purple-50 text-purple-800' : 'border-blue-200 bg-blue-50 text-blue-800';
  return <div className="mb-4 grid gap-2 sm:grid-cols-3"><div className={cx('rounded-xl border px-3 py-2 text-xs font-bold', accent)}><span className="mono mr-2 text-[10px] opacity-65">01</span>{t('checkoutStepOne')}</div><div className="rounded-xl border border-[#dbe7e9] bg-white px-3 py-2 text-xs font-bold text-slate-600"><span className="mono mr-2 text-[10px] text-slate-400">02</span>{t('checkoutStepTwo')}</div><div className="rounded-xl border border-[#dbe7e9] bg-white px-3 py-2 text-xs font-bold text-slate-600"><span className="mono mr-2 text-[10px] text-slate-400">03</span>{t('checkoutStepThree')}</div></div>;
}
...
function ProxyOrderForm({ product, isTrial }: { product: CatalogProduct; isTrial?: boolean }) {
  ...
  const fieldClass = 'min-h-10 w-full rounded-xl border border-[#dbe7e9] bg-white px-2.5 py-2 text-xs font-semibold text-[#142037] outline-none transition focus:border-[#f46c43] focus:ring-2 focus:ring-[#f46c43]/10';
  ...
  return <>
    <tr className="border-b border-[#edf2f3] last:border-0">
      ...
      <td className="px-4 py-4 text-xs font-extrabold text-[#13716e]"><div className="whitespace-nowrap">{unitCreditCost === null ? '—' : formatCredits(unitCreditCost)}<span className="ml-1 font-normal text-slate-400">{t('perDay')}</span></div>{unitIdrPrice !== null && unitCreditCost !== null && <p className="mt-1 max-w-[180px] whitespace-normal text-[10px] font-medium leading-4 text-slate-400">{formatCredits(unitCreditCost)} ≈ {formatUsd(product.unitPrice)} ≈ {formatIdr(unitIdrPrice)}</p>}</td>
              <td className="w-24 px-3 py-4"><select aria-label={`Nodes for ${country}`} className={fieldClass} value={nodeCount} disabled={isTrial} onChange={event => setNodeCount(Number(event.target.value))}>{isTrial ? <option value={1}>1</option> : [1, 5, 10, 20, 30].map(value => <option key={value} value={value}>{value}</option>)}</select></td>
      ...
      <td className="w-36 px-3 py-4"><span className="inline-flex min-h-9 items-center rounded-lg border border-[#bfe3df] bg-[#eaf8f6] px-2.5 text-xs font-bold text-[#13716e]">{t('credit')}</span></td>
      <td className="whitespace-nowrap px-4 py-4 text-sm font-extrabold text-[#142037]">{quote.isLoading ? '…' : <>{formatCredits(quote.data?.creditCost || 0)}<span className="mt-1 block text-[10px] font-medium text-[#13716e]">{creditBalance.data?.balance?.toLocaleString(locale === 'id' ? 'id-ID' : 'en-US') || 0} {t('creditAvailable')}</span></>}</td>
      <td className="px-4 py-4"><Button className={cx('min-h-10 whitespace-nowrap px-3 text-xs !text-white', product.proxyType === 'residential' ? '!bg-purple-600 hover:!bg-purple-700' : '!bg-blue-600 hover:!bg-blue-700')} disabled={isTrial === undefined || residentialTrialBlocked || createOrder.isPending || quote.isLoading || !quote.data} onClick={submit}>{createOrder.isPending ? t('creating') : product.proxyType === 'residential' ? t('orderResidential') : t('orderDatacenter')}</Button></td>
    </tr>
...
function TempMailWorkspace() {
  const domains = useTempMailDomains({ query: { staleTime: 5 * 60_000 } });
  const [draftName, setDraftName] = useState('');
  const [draftDomain, setDraftDomain] = useState('');
  const [address, setAddress] = useState('');
  const [selectedId, setSelectedId] = useState<string | null>(null);
  const messages = useTempMailMessages(address, { query: { enabled: Boolean(address) } });
  const randomName = () => {
    const names = ['alex', 'aria', 'dimas', 'farah', 'hana', 'james', 'kai', 'lina', 'maya', 'noah', 'raka', 'sara'];
    const values = crypto.getRandomValues(new Uint32Array(2));
    return `${names[values[0] % names.length]}${String(values[1] % 10_000).padStart(4, '0')}`;
  };
  useEffect(() => {
    const firstDomain = domains.data?.domains[0];
    if (!firstDomain || address) return;
    const name = randomName();
    setDraftName(name); setDraftDomain(firstDomain); setAddress(`${name}@${firstDomain}`);
  }, [domains.data?.domains, address]);
  const createMailbox = () => {
    if (!draftDomain || !/^[a-z0-9][a-z0-9._+-]{0,63}$/i.test(draftName)) return;
    setAddress(`${draftName}@${draftDomain}`); setSelectedId(null);
  };
  const randomMailbox = () => { const name = randomName(); setDraftName(name); if (draftDomain) { setAddress(`${name}@${draftDomain}`); setSelectedId(null); } };
  const selected = messages.data?.messages.find(item => item.id === selectedId) || messages.data?.messages[0];
  const body = selected?.text || selected?.html?.replace(/<[^>]*>/g, ' ').replace(/\s+/g, ' ').trim() || '';
  return <CustomerShell active="temp-mail"><main className="mx-auto max-w-7xl px-5 py-9 pb-28 lg:px-8 lg:py-12"><section className="rounded-3xl bg-[#142037] px-6 py-6 text-white"><p className="mono text-[10px] uppercase tracking-[.2em] text-[#69d5d0]">private workspace tool</p><div className="mt-2 flex flex-wrap items-end justify-between gap-4"><div><h1 className="text-3xl font-extrabold tracking-[-.05em]">Temp Mail</h1><p className="mt-2 text-sm text-slate-300">Disposable inboxes for low-risk sign-up flows. Messages refresh automatically every 15 seconds.</p></div><Mail className="text-[#69d5d0]" size={30} /></div></section><section className="mt-6 grid gap-5 xl:grid-cols-[360px_1fr]"><aside className="rounded-3xl border border-[#dbe7e9] bg-white p-6 shadow-[0_10px_35px_rgba(20,32,55,.04)]"><p className="mono text-[10px] uppercase tracking-[.16em] text-[#e4643d]">mailbox composer</p><h2 className="mt-2 text-xl font-extrabold">Create mailbox</h2><label className="mt-6 block text-xs font-bold text-slate-500">Mailbox name<input value={draftName} maxLength={64} onChange={event => setDraftName(event.target.value.toLowerCase().replace(/[^a-z0-9._+-]/g, ''))} className="mt-2 block w-full rounded-xl border border-[#dbe7e9] bg-[#f8fbfb] px-3 py-3 text-sm font-semibold text-[#142037] outline-none focus:border-[#69d5d0]" /></label><label className="mt-4 block text-xs font-bold text-slate-500">Domain<select value={draftDomain} onChange={event => setDraftDomain(event.target.value)} className="mt-2 block w-full rounded-xl border border-[#dbe7e9] bg-[#f8fbfb] px-3 py-3 text-sm font-semibold text-[#142037] outline-none focus:border-[#69d5d0]">{domains.data?.domains.map(value => <option key={value} value={value}>{value}</option>)}</select></label><Button className="mt-5 w-full" disabled={!draftName || !draftDomain} onClick={createMailbox}><Mail size={15} /> Create mailbox</Button><Button variant="outline" className="mt-2 w-full" disabled={!draftDomain} onClick={randomMailbox}><RefreshCw size={15} /> Random name</Button><div className="mt-6 rounded-2xl border border-[#bfe3df] bg-[#eaf8f6] p-4"><p className="mono text-[9px] uppercase tracking-[.14em] text-[#13716e]">active address</p><p className="mt-2 break-all text-sm font-extrabold">{address || 'Loading…'}</p><button className="mt-3 inline-flex items-center gap-1 text-xs font-bold text-[#e05c37]" onClick={() => void navigator.clipboard?.writeText(address)}><Copy size={13} /> Copy address</button></div><p className="mt-5 text-xs leading-5 text-slate-500">Never use temporary mail for passwords, payments, or account recovery.</p></aside><section className="overflow-hidden rounded-3xl border border-[#dbe7e9] bg-white shadow-[0_10px_35px_rgba(20,32,55,.04)]"><div className="flex items-center justify-between border-b border-[#edf2f3] px-5 py-4"><div><p className="mono text-[10px] uppercase tracking-[.16em] text-[#e4643d]">inbox</p><h2 className="mt-1 text-lg font-extrabold">{messages.data?.messages.length || 0} messages</h2></div><Button variant="outline" className="min-h-9 px-3 text-xs" disabled={!address || messages.isFetching} onClick={() => void messages.refetch()}><RefreshCw size={14} className={messages.isFetching ? 'animate-spin' : ''} /> Refresh</Button></div><State loading={domains.isLoading || (Boolean(address) && messages.isLoading)} error={domains.isError || messages.isError} onRetry={() => { void domains.refetch(); void messages.refetch(); }}><div className="grid min-h-[480px] md:grid-cols-[.75fr_1.25fr]"><div className="max-h-[480px] overflow-y-auto border-b border-[#edf2f3] md:border-b-0 md:border-r">{messages.data?.messages.length ? messages.data.messages.map(message => <button key={message.id} onClick={() => setSelectedId(message.id)} className={cx('block w-full border-b border-[#edf2f3] px-5 py-4 text-left', selected?.id === message.id ? 'bg-[#eaf8f6]' : 'hover:bg-[#f8fbfb]')}><p className="truncate text-sm font-extrabold">{message.subject}</p><p className="mt-1 truncate text-xs text-slate-500">{message.from}</p></button>) : <div className="grid h-full place-items-center p-8 text-center"><Mail className="text-[#69d5d0]" size={28} /><p className="mt-3 text-sm font-bold">Inbox is waiting</p><p className="mt-1 text-xs leading-5 text-slate-500">Create an address, then use it to receive email.</p></div>}</div><article className="min-w-0 p-5">{selected ? <><h3 className="text-lg font-extrabold">{selected.subject}</h3><p className="mt-2 text-xs text-slate-500">From: {selected.from}</p><pre className="mt-5 whitespace-pre-wrap break-words font-sans text-sm leading-6 text-slate-700">{body || 'This message has no readable text body.'}</pre></> : <div className="grid h-full place-items-center text-center text-sm text-slate-500">Choose a message to read it here.</div>}</article></div></State></section></section></main></CustomerShell>;
}
...
function StaticResidentialPage() {
  ...
  const replace = useReplaceStaticResidentialNode();
  ...
  const isExpiredOrder = (order: StaticResidentialOrder) => order.status === 'expired' || new Date(order.expiresAt) <= new Date();
  ...
  const download = () => exporter.mutate(undefined, { onSuccess: ({ filename, content, count }) => { if (!count) return window.alert(t('noActiveStaticPorts')); const url = URL.createObjectURL(new Blob([content], { type: 'text/plain;charset=utf-8' })); const anchor = document.createElement('a'); anchor.href = url; anchor.download = filename; anchor.click(); URL.revokeObjectURL(url); }, onError: error => window.alert(error.message) });
  return <CustomerShell active="static"><main className="mx-auto max-w-7xl px-5 py-9 pb-28 lg:px-8 lg:py-12">
    <section className="relative overflow-hidden rounded-3xl bg-[#142037] px-5 py-5 text-white md:px-6"><div className="hero-grid absolute inset-0 opacity-15" /><div className="relative flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><Link href="/client" className="mono text-[10px] uppercase tracking-[.18em] text-[#69d5d0] hover:text-white">{t('services')} / {t('clientNavStatic')}</Link><h1 className="mt-2 text-2xl font-extrabold tracking-[-.05em] md:text-3xl">{t('staticTitle')}</h1><p className="mt-2 max-w-xl text-xs leading-5 text-slate-300 md:text-sm">{t('staticIntro')}</p></div><a href="#checkout" className="inline-flex min-h-9 items-center justify-center self-start rounded-xl bg-[#f46c43] px-3 text-xs font-bold text-white hover:bg-[#df5934]">{t('orderFivePorts')} <ArrowRight className="ml-2" size={14} /></a></div></section>
    <section className="mt-6 grid gap-4 sm:grid-cols-3"><Metric label={t('fixedPorts')} value="5" detail={t('perAccountOrder')} icon={Network} tone="teal" /><Metric label={t('sharedTraffic')} value={`${quotaGb} GB`} detail={t('inboundOutbound')} icon={Gauge} tone="orange" /><Metric label={t('staticIp')} value={t('fixedIp')} detail={t('noAutomaticRotation')} icon={ShieldCheck} tone="teal" /></section>
    <section id="checkout" className="scroll-mt-24 pt-12"><SectionTitle eyebrow={t('creditCheckout')} title={t('staticCheckoutTitle')} body={t('staticCheckoutBody')} /><div className="grid gap-5 lg:grid-cols-[.8fr_1.2fr]"><div className="rounded-3xl border border-[#dbe7e9] bg-white p-6"><p className="mono text-[10px] uppercase tracking-[.16em] text-[#e4643d]">{t('fixedPackage')}</p><p className="mt-3 text-2xl font-extrabold">5 {t('fixedPorts').toLowerCase()} · {quotaGb}GB {t('sharedQuota')}</p><p className="mt-2 text-sm leading-6 text-slate-500">{t('staticPackageBody')}</p><label className="mt-5 block text-sm font-bold">{t('chooseQuota')}<div className="mt-2 grid grid-cols-3 gap-2">{([1, 3, 5] as const).map(value => <button key={value} type="button" disabled={Boolean(activeStaticOrder)} onClick={() => setQuotaGb(value)} className={cx('min-h-10 rounded-xl border px-3 text-sm font-extrabold transition disabled:cursor-not-allowed disabled:opacity-60', quotaGb === value ? 'border-[#f46c43] bg-[#fff2ed] text-[#d94f2c]' : 'border-[#dbe7e9] text-slate-600 hover:border-[#f46c43]')}>{value} GB</button>)}</div></label><label className="mt-5 block text-sm font-bold">{t('rentalDays')}<select disabled={Boolean(activeStaticOrder)} className="mt-2 block w-full rounded-xl border border-[#dbe7e9] bg-[#f8fbfb] px-3 py-3 text-sm disabled:cursor-not-allowed disabled:opacity-60" value={days} onChange={event => setDays(Number(event.target.value))}>{[1, 3, 7, 15, 30].map(value => <option key={value} value={value}>{value} {t(value === 1 ? 'day' : 'daysLabel')}</option>)}</select></label></div><div className="rounded-3xl bg-white p-6 shadow-[0_10px_35px_rgba(20,32,55,.04)]"><State loading={quote.isLoading} error={quote.isError} onRetry={() => quote.refetch()}><div className="grid gap-4 sm:grid-cols-2"><div><p className="text-xs font-bold text-slate-500">{t('pricePerGbDay')}</p><p className="mt-1 text-lg font-extrabold">${quote.data?.pricePerGbDay.toFixed(4) || '—'}</p></div><div><p className="text-xs font-bold text-slate-500">{t('total')}</p><p className="mt-1 text-lg font-extrabold">{quote.data ? `${quote.data.creditCost.toLocaleString()} ${t('credit').toLowerCase()}` : '—'}</p></div><div><p className="text-xs font-bold text-slate-500">{t('availableInventory')}</p><p className="mt-1 text-lg font-extrabold">{quote.data?.availableNodes ?? '—'} {t('upstreams')}</p></div><div><p className="text-xs font-bold text-slate-500">{t('yourCreditBalance')}</p><p className="mt-1 text-lg font-extrabold text-[#13716e]">{(credit.data?.balance || 0).toLocaleString()} cr</p></div></div><Button className="mt-6 w-full" disabled={identity.isLoading || isTrial || Boolean(activeStaticOrder) || create.isPending || !quote.data?.canFulfill || (credit.data?.balance || 0) < (quote.data?.creditCost || Infinity)} onClick={() => create.mutate({ data: { rentalDays: days, quotaGb } }, { onSuccess: refresh, onError: error => window.alert(error.message) })}>{isTrial ? t('staticTrialBlocked') : activeStaticOrder ? `${t('activeOrderExists')} #${activeStaticOrder.id}` : create.isPending ? t('provisioning') : t('payCreatePorts')}</Button>{activeStaticOrder ? <p className="mt-3 text-xs font-semibold text-[#b75d12]">{t('activeStaticOrderNotice')}</p> : isTrial ? <p className="mt-3 text-xs font-semibold text-[#b75d12]">{t('staticTrialNotice')}</p> : quote.data && !quote.data.canFulfill && <p className="mt-3 text-xs font-semibold text-red-600">{t('inventoryRequired')}</p>}</State></div></div></section>
    <section className="scroll-mt-24 pt-14"><SectionTitle eyebrow="my static ports" title="Connection details" body="Only your account SOCKS5 credential is shown. Upstream provider credentials are never returned to the browser." action={<Button variant="outline" disabled={exporter.isPending} onClick={download}><Download size={15} />{exporter.isPending ? 'Preparing…' : 'Download TXT'}</Button>} /><State loading={orders.isLoading} error={orders.isError} onRetry={() => orders.refetch()} empty={!orders.data?.length}><div className="grid gap-5">{orders.data?.map(order => {
      const expired = isExpiredOrder(order);
      return <div key={order.id} className="rounded-3xl border border-[#dbe7e9] bg-white p-5"><div className="flex flex-wrap items-start justify-between gap-4"><div><div className="flex items-center gap-2"><h2 className="font-extrabold">Order #{order.id}</h2><Badge tone={order.status === 'active' && !expired ? 'green' : order.status === 'quota_exceeded' ? 'red' : 'neutral'}>{expired ? 'expired' : order.status.replace('_', ' ')}</Badge></div><p className="mt-1 text-sm text-slate-500">Expires {date(order.expiresAt)} · {bytes(order.usedBytes)} / {bytes(order.quotaBytes)} used</p><p className="mt-1 text-xs font-semibold text-[#13716e]">{t('ipReplacements')}: {order.replacementCount}/5 · {order.replacementsRemaining} {t('remaining')}</p></div><Button variant="outline" className="min-h-9 px-3 text-xs" onClick={() => setExtending(order)}>Extend</Button></div><div className="mt-4 h-2 overflow-hidden rounded-full bg-[#edf2f3]"><div className="h-full rounded-full bg-[#69d5d0]" style={{ width: `${Math.min(100, (order.usedBytes / order.quotaBytes) * 100)}%` }} /></div>{expired ? <div className="mt-5 rounded-2xl border border-dashed border-[#dbe7e9] bg-[#f8fbfb] px-4 py-5 text-sm font-semibold text-slate-500">This order has expired. Extend it to show and reuse its static residential ports.</div> : <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-5">{order.nodes.map(node => <StaticResidentialNodeCard key={node.id} node={node} replacementsRemaining={order.replacementsRemaining} replacing={replace.isPending} onReplace={() => replace.mutate({ orderId: order.id, nodeId: node.id }, { onSuccess: refresh, onError: error => window.alert(error.message) })} />)}</div>}</div>;
    })}</div></State></section>
  </main>{extending && <Modal title={`Extend static order #${extending.id}`} onClose={() => setExtending(null)}><p className="text-sm leading-6 text-slate-500">This adds time to your existing five public ports. The shared quota remains {bytes(extending.quotaBytes)}.</p><label className="mt-5 block text-sm font-bold">Additional days<select className="mt-2 block w-full rounded-xl border border-[#dbe7e9] bg-[#f8fbfb] px-3 py-3 text-sm" value={days} onChange={event => setDays(Number(event.target.value))}>{[1, 3, 7, 15, 30].map(value => <option key={value} value={value}>{value} day{value === 1 ? '' : 's'}</option>)}</select></label><div className="mt-6 flex gap-3"><Button variant="outline" className="flex-1" onClick={() => setExtending(null)}>Cancel</Button><Button className="flex-1" disabled={extend.isPending} onClick={() => extend.mutate({ id: extending.id, data: { rentalDays: days } }, { onSuccess: () => { setExtending(null); refresh(); }, onError: error => window.alert(error.message) })}>{extend.isPending ? 'Extending…' : 'Pay & extend'}</Button></div></Modal>}</CustomerShell>;
}

function StaticResidentialNodeCard({ node, replacementsRemaining, replacing, onReplace }: { node: StaticResidentialOrder['nodes'][number]; replacementsRemaining: number; replacing: boolean; onReplace: () => void }) {
  const { t } = useLocalePreferences();
  ...
  const connectionLabel = connection ? `${connection.host}:${connection.port}` : t('unavailable');
  return <div className="min-w-0 rounded-2xl border border-[#e1eaec] border-t-2 border-t-[#69d5d0] bg-[#f8fbfb] p-3"><div className="flex items-center justify-between gap-2"><p className="mono text-[10px] font-bold text-slate-500">{t('port').toUpperCase()} {node.port}</p><Badge tone={node.status === 'active' ? 'green' : 'neutral'}>{node.status}</Badge></div><div className="mt-3"><p className="text-[9px] font-bold uppercase tracking-[.12em] text-slate-400">Residential IP</p><p className="mt-1 truncate text-sm font-extrabold text-[#142037]" title={node.egressIp || undefined}>{node.egressIp || '—'}</p><p className="mt-1 truncate mono text-[10px] text-slate-400" title={connectionLabel}>{connectionLabel}</p></div><div className="mt-3 flex items-center justify-between gap-2 border-t border-[#e1eaec] pt-3"><p className="min-w-0 truncate text-[10px] text-slate-500" title={node.lastReplacedAt ? `${t('lastIpReplacement')}: ${date(node.lastReplacedAt)}` : t('fixedIpNoRotation')}>{node.lastReplacedAt ? `${t('lastIpReplacement')}: ${date(node.lastReplacedAt)}` : t('fixedIpNoRotation')}</p><div className="flex shrink-0 items-center gap-1"><button type="button" title="Copy SOCKS5" aria-label="Copy SOCKS5" disabled={!socks5Value} onClick={() => void navigator.clipboard?.writeText(socks5Value)} className="grid h-8 w-8 place-items-center rounded-lg border border-[#dbe7e9] bg-white text-[#13716e] transition hover:border-[#69d5d0] hover:bg-[#eaf8f6] disabled:cursor-not-allowed disabled:opacity-40"><Copy size={14} /></button><button type="button" title="Copy HTTP" aria-label="Copy HTTP" disabled={!httpValue} onClick={() => void navigator.clipboard?.writeText(httpValue)} className="grid h-8 w-8 place-items-center rounded-lg border border-[#dbe7e9] bg-white text-[#7c4dcc] transition hover:border-[#bca7e9] hover:bg-[#f3effd] disabled:cursor-not-allowed disabled:opacity-40"><Link2 size={14} /></button><button type="button" title={replacing ? t('replacingIp') : `${t('replaceIp')} (${replacementsRemaining}/5)`} aria-label={t('replaceIp')} disabled={!replacementsRemaining || replacing} onClick={onReplace} className="grid h-8 w-8 place-items-center rounded-lg border border-[#dbe7e9] bg-white text-[#e05c37] transition hover:border-[#f46c43] hover:bg-[#fff2ed] disabled:cursor-not-allowed disabled:opacity-40"><RefreshCw size={14} className={replacing ? 'animate-spin' : ''} /></button></div></div></div>;
}
...
function ClientPortalPage({ serviceType }: { serviceType?: 'datacenter' | 'residential' }) {
  ...
  const datacenterNodes = visibleNodes.filter(node => orderById.get(String(node.orderId))?.proxyType !== 'residential');
  const residentialNodes = visibleNodes.filter(node => orderById.get(String(node.orderId))?.proxyType === 'residential');
  const activeResidentialNodeTotal = activeOrders
    .filter(order => order.proxyType === 'residential')
    .reduce((total, order) => total + order.nodeCount, 0);
  const activeResidentialOrders = activeOrders.filter(order => order.proxyType === 'residential');
  const activeDatacenterOrders = activeOrders.filter(order => order.proxyType !== 'residential');
  const failedResidentialNodeTotal = (runtimeNodes.data || []).filter(node => (
    failedOrderIds.has(String(node.orderId))
    && orderById.get(String(node.orderId))?.proxyType === 'residential'
    && node.status !== 'online'
  ...
  const requestRecreateAll = (proxyType?: 'residential') => {
    ...
    const productLabel = proxyType === 'residential' ? ` ${t('residential').toLowerCase()}` : '';
    const message = `Force recreate ${nodeTotal} eligible${productLabel} node${nodeTotal === 1 ? '' : 's'}? Active proxies will be temporarily unavailable and receive a new sandbox/IP. Failed provisioning nodes will be queued again.`;
    if (!window.confirm(message)) return;
    recreateAll.mutate(proxyType ? { proxyType } : undefined, {
      onSuccess: result => {
        void qc.invalidateQueries({ queryKey: getListClientProxyNodesQueryKey() });
        void qc.invalidateQueries({ queryKey: getGetClientOverviewQueryKey() });
        window.alert(`Recreation queued for ${result.nodeIds.length}${productLabel} node${result.nodeIds.length === 1 ? '' : 's'}.`);
      },
      onError: error => window.alert(error.message),
    });
  };
  ...
  return <CustomerShell active={activeProxyTab}>
    <main className="mx-auto max-w-7xl px-5 py-9 lg:px-8 lg:py-12">
      <section className="relative overflow-hidden rounded-3xl bg-[#142037] px-5 py-5 text-white md:px-6"><div className="hero-grid absolute inset-0 opacity-15" /><div className="relative flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><Link href="/dashboard" className="mono text-[10px] uppercase tracking-[.18em] text-[#69d5d0] hover:text-white">Dashboard / SOCKS5</Link><span className={cx('mt-3 inline-flex rounded-full px-3 py-1 text-xs font-extrabold', activeProxyTab === 'residential' ? 'bg-purple-500/25 text-purple-100 ring-1 ring-purple-300/40' : 'bg-blue-500/25 text-blue-100 ring-1 ring-blue-300/40')}>{activeProxyTab === 'residential' ? t('residentialSocks') : t('datacenterSocks')}</span><h1 className="mt-2 text-2xl font-extrabold tracking-[-.05em] md:text-3xl">{activeProxyTab === 'residential' ? t('residentialSocks') : t('socksByCountry')}</h1><p className="mt-2 max-w-xl text-xs leading-5 text-slate-300 md:text-sm">{activeProxyTab === 'residential' ? t('residentialTabBody') : t('datacenterTabBody')}</p></div><a href={activeProxyTab === 'residential' ? '#residential-catalog' : '#catalog'} className="inline-flex min-h-9 items-center justify-center gap-2 self-start rounded-xl bg-[#f46c43] px-3 text-xs font-bold text-white hover:bg-[#df5934]">{activeProxyTab === 'residential' ? t('orderResidential') : t('orderDatacenter')} <ArrowRight size={14} /></a></div></section>
      ...
      {activeProxyTab === 'datacenter' && <><div className="pt-8"><ProxyCheckoutGuide proxyType="datacenter" /></div>
      ...
      {activeProxyTab === 'residential' && <><div className="pt-8"><ProxyCheckoutGuide proxyType="residential" /></div>
...
function TurnstileChallenge({ onToken }: { onToken: (token: string) => void }) {
  ...
  if (!siteKey) return import.meta.env.PROD
    ? <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-xs text-amber-800">{t('registrationProtectionMissing')}</div>
    : null;
...
function AuthPage({ mode = 'sign-in' }: { mode?: 'sign-in' | 'sign-up' }) {
  ...
  const submit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setError(caught instanceof Error ? caught.message : isSignup ? t('registrationFailed') : t('authenticationFailed'));
    } finally {
  ...
  return <div className="grid min-h-[100dvh] place-items-center bg-[#eaf3f3] px-4 py-10"><div className="absolute left-6 top-6"><Logo /></div><div className="w-full max-w-[440px] rounded-3xl bg-white p-7 shadow-xl sm:p-9"><p className="mono text-[10px] uppercase tracking-[.18em] text-[#e4643d]">{t('secureWorkspace')}</p><h1 className="mt-3 text-3xl font-extrabold tracking-[-.05em] text-[#142037]">{isSignup ? t('createYourAccount') : t('welcomeBack')}</h1><p className="mt-2 text-sm text-slate-500">{isSignup ? t('signupBody') : t('signinBody')}</p><form onSubmit={submit} className="mt-7 grid gap-4">{isSignup && <label className="text-sm font-bold text-[#142037]">{t('name')}<input required minLength={2} maxLength={80} autoComplete="name" value={name} onChange={event => setName(event.target.value)} className="mt-2 block w-full rounded-xl border border-[#d9e2e6] bg-[#f3f7f8] px-4 py-3 font-normal outline-none focus:border-[#f46c43]" /></label>}<label className="text-sm font-bold text-[#142037]">{t('email')}<input required type="email" autoComplete="email" value={email} onChange={event => setEmail(event.target.value)} className="mt-2 block w-full rounded-xl border border-[#d9e2e6] bg-[#f3f7f8] px-4 py-3 font-normal outline-none focus:border-[#f46c43]" /></label><label className="text-sm font-bold text-[#142037]">{t('password')}<input required type="password" minLength={10} autoComplete={isSignup ? 'new-password' : 'current-password'} value={password} onChange={event => setPassword(event.target.value)} className="mt-2 block w-full rounded-xl border border-[#d9e2e6] bg-[#f3f7f8] px-4 py-3 font-normal outline-none focus:border-[#f46c43]" /></label><TurnstileChallenge key={captchaAttempt} onToken={setCaptchaToken} />{error && <div className="rounded-xl border border-red-100 bg-red-50 px-4 py-3 text-sm text-red-700">{error}</div>}{message && <div className="rounded-xl border border-emerald-100 bg-emerald-50 px-4 py-3 text-sm text-emerald-800">{message}</div>}<Button type="submit" disabled={pending || (turnstileRequired && !captchaToken)} className="mt-1 w-full">{pending ? t('pleaseWait') : isSignup ? t('createAccount') : t('signIn')}</Button></form><div className="mt-6 border-t border-[#edf2f3] pt-5 text-center"><p className="text-xs leading-5 text-slate-500">{isSignup ? t('alreadyHaveAccount') : t('newToNodenesia')}</p><Link href={isSignup ? '/sign-in' : '/sign-up'} className="mt-2 inline-flex text-sm font-bold text-[#d95432] hover:text-[#b33f24]">{isSignup ? t('signIn') : t('createAccount')}</Link><CommunityLinks className="mt-3" /></div></div></div>;
}
...
function TelegramVerificationPage() {
  ...
  const error = start.error?.message || (identity.isError || status.isError ? t('loadErrorTitle') : '');
  return <div className="grid min-h-[100dvh] place-items-center bg-[#eaf3f3] px-4 py-10"><div className="absolute left-6 top-6"><Logo /></div><div className="w-full max-w-lg rounded-3xl bg-white p-7 shadow-xl sm:p-9"><div className="grid h-12 w-12 place-items-center rounded-2xl bg-[#def5f3] text-[#13716e]"><Send size={22} /></div><p className="mono mt-6 text-[10px] uppercase tracking-[.18em] text-[#e4643d]">{t('accountVerification')}</p><h1 className="mt-3 text-3xl font-extrabold tracking-[-.05em] text-[#142037]">{t('verifyTelegram')}</h1><p className="mt-3 text-sm leading-6 text-slate-500">{t('verifyTelegramBody')}</p><div className="mt-6 grid gap-3 rounded-2xl bg-[#f4f8f8] p-4 text-sm text-slate-600"><p><strong className="text-[#142037]">1.</strong> {t('telegramStepOne')}</p><p><strong className="text-[#142037]">2.</strong> {t('telegramStepTwo')}</p><p><strong className="text-[#142037]">3.</strong> {t('telegramStepThree')}</p></div>{error && <div className="mt-5 rounded-xl border border-red-100 bg-red-50 px-4 py-3 text-sm text-red-700">{error}</div>}{link ? <div className="mt-6"><a href={link.telegramUrl} target="_blank" rel="noreferrer" className="inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-xl bg-[#229ED9] px-4 text-sm font-bold text-white hover:bg-[#168ac1]"><Send size={16} /> {t('continueTelegram')}</a><p className="mt-2 text-center text-xs text-slate-400">{t('linkExpires')} {new Date(link.expiresAt).toLocaleTimeString()}</p><Button variant="outline" className="mt-3 w-full" onClick={() => { void identity.refetch(); void status.refetch(); }}><RefreshCw size={15} /> {t('checkVerification')}</Button></div> : <Button className="mt-6 w-full" disabled={start.isPending} onClick={() => start.mutate()}>{start.isPending ? <><RefreshCw size={15} className="animate-spin" /> {t('preparing')}</> : <><Send size={15} /> {t('connectTelegram')}</>}</Button>}<button className="mx-auto mt-6 block text-xs font-bold text-slate-500 hover:text-[#142037]" onClick={() => void signOut().then(() => { queryClient.clear(); setLocation('/'); })}>{t('signOut')}</button></div></div>;
}
...
type ProtectedPage = 'client-dashboard' | 'client-proxy' | 'client-residential' | 'client-static-residential' | 'client-temp-mail' | 'client-orders' | 'client-nodes' | 'client-billing' | 'admin-security' | 'admin-overview' | 'admin-catalog' | 'admin-info-users' | 'admin-credits' | 'admin-payment-invoices' | 'admin-proxy-api-keys' | 'admin-proxy-providers' | 'admin-proxy-orders' | 'admin-proxy-provisioning-logs' | 'admin-proxy-settings' | 'admin-static-residential' | 'admin-settings';
...
function Protected({ page }: { page: ProtectedPage }) {
  ...
  switch (page) {
    ...
    case 'client-temp-mail': return <TempMailWorkspace />;
    ...
    case 'admin-payment-invoices': return <AdminPaymentInvoicesPage />;
...
function RouterViews() {
  return <RoutedErrorBoundary><Switch><Route path="/" component={HomeRedirect} /><Route path="/sign-in/*?" component={() => <AuthPage mode="sign-in" />} /><Route path="/sign-up/*?" component={() => <AuthPage mode="sign-up" />} /><Route path="/verify-telegram" component={TelegramVerificationPage} /><Route path="/dashboard/datacenter" component={() => <Protected page="client-proxy" />} /><Route path="/dashboard/residential" component={() => <Protected page="client-residential" />} /><Route path="/dashboard/static-residential" component={() => <Protected page="client-static-residential" />} /><Route path="/dashboard/temp-mail" component={() => <Protected page="client-temp-mail" />} /><Route path="/dashboard/orders" component={() => <Protected page="client-orders" />} /><Route path="/dashboard/nodes" component={() => <Protected page="client-nodes" />} /><Route path="/dashboard/billing" component={() => <Protected page="client-billing" />} /><Route path="/dashboard" component={() => <Protected page="client-dashboard" />} /><Route path="/client/nodes" component={() => <Redirect to="/dashboard/nodes" />} /><Route path="/client/orders" component={() => <Redirect to="/dashboard/orders" />} /><Route path="/client/proxy" component={() => <Redirect to="/dashboard/datacenter" />} /><Route path="/client/static-residential" component={() => <Redirect to="/dashboard/static-residential" />} /><Route path="/client" component={() => <Redirect to="/dashboard" />} /><Route path="/admin/security" component={() => <Protected page="admin-security" />} /><Route path="/admin/users" component={() => <Redirect to="/admin/info/users" />} /><Route path="/admin/credits" component={() => <Protected page="admin-credits" />} /><Route path="/admin/payment-invoices" component={() => <Protected page="admin-payment-invoices" />} /><Route path="/admin/keys" component={() => <Redirect to="/admin/proxy/api-keys" />} /><Route path="/admin/orders" component={() => <Redirect to="/admin/proxy/orders" />} /><Route path="/admin/info/users" component={() => <Protected page="admin-info-users" />} /><Route path="/admin/proxy/api-keys" component={() => <Protected page="admin-proxy-api-keys" />} /><Route path="/admin/proxy/providers" component={() => <Protected page="admin-proxy-providers" />} /><Route path="/admin/proxy/orders" component={() => <Protected page="admin-proxy-orders" />} /><Route path="/admin/proxy/provisioning-logs" component={() => <Protected page="admin-proxy-provisioning-logs" />} /><Route path="/admin/proxy/settings" component={() => <Protected page="admin-proxy-settings" />} /><Route path="/admin/static-residential" component={() => <Protected page="admin-static-residential" />} /><Route path="/admin/settings" component={() => <Protected page="admin-settings" />} /><Route path="/admin/catalog" component={() => <Protected page="admin-catalog" />} /><Route path="/admin" component={() => <Protected page="admin-overview" />} /><Route component={NotFound} /></Switch></RoutedErrorBoundary>;
}
