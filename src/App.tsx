  ...
  return <div className="min-h-[100dvh] bg-[#f4f8f8] text-[#142037]">
    ...
    <main className="mx-auto max-w-7xl px-5 py-9 pb-28 lg:px-8 lg:py-12">
      ...
      <section id="my-services" className="scroll-mt-24 pt-16"><SectionTitle eyebrow={t('portfolio')} title={t('myNodes')} body={t('myNodesBody')} action={<div className="flex flex-wrap gap-2"><Button variant="outline" disabled={!visibleNodes.length || exportConnections.isPending} onClick={downloadConnections}><Download size={15} />{exportConnections.isPending ? t('preparing') : t('download')}</Button><Button variant="danger" disabled={!recreateNodeTotal || recreateAll.isPending} onClick={requestRecreateAll} data-testid="button-force-recreate-all-nodes"><RefreshCw size={15} className={recreateAll.isPending ? 'animate-spin' : ''} />{recreateAll.isPending ? t('recreating') : `${t('forceRecreate')} (${recreateNodeTotal})`}</Button></div>} />
        ...
        <State loading={orders.isLoading || runtimeNodes.isLoading} error={orders.isError || runtimeNodes.isError} onRetry={() => { void orders.refetch(); void runtimeNodes.refetch(); }} empty={!visibleNodes.length}><div className="space-y-7">{([[t('datacenterSocks'), datacenterNodes], [t('residentialNodes'), residentialNodes]] as const).map(([title, nodes]) => nodes.length ? <div key={title}><p className="mono mb-3 text-[10px] font-bold uppercase tracking-[.16em] text-slate-400">{title} · {nodes.length} {t('nodes').toLowerCase()}</p><div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{nodes.map(node => { const order = orderById.get(String(node.orderId))!; return <div key={node.id} className="min-w-0"><div className="mb-1.5 flex items-center justify-between px-0.5"><p className="truncate text-[11px] font-bold text-[#142037]">{order.productName}</p><span className="mono ml-2 shrink-0 text-[8px] uppercase text-slate-400">Order #{order.id} · node {node.id}</span></div><ActiveNodeItem order={order} node={node} /></div>; })}</div></div> : null)}</div></State>
      </section>
