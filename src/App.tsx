...
function LandingPlanCard({ name, price, description, nodeAllowance, durationAllowance, highlighted = false }: { name: string; price: string; description: string; nodeAllowance: string; durationAllowance: string; highlighted?: boolean }) {
  return <article className={cx('relative rounded-3xl border p-6', highlighted ? 'border-[#f46c43] bg-[#142037] text-white shadow-xl shadow-slate-900/10' : 'border-[#dbe7e9] bg-white text-[#142037]')}>
    ...
    <dl className={cx('mt-6 grid gap-3 border-t pt-5 text-sm', highlighted ? 'border-white/10 text-slate-200' : 'border-slate-100 text-slate-600')}><div className="flex items-center gap-2"><Check size={15} className="shrink-0 text-[#69d5d0]" /><dt className="sr-only">Nodes</dt><dd>{nodeAllowance}</dd></div><div className="flex items-center gap-2"><Check size={15} className="shrink-0 text-[#69d5d0]" /><dt className="sr-only">Duration</dt><dd>{durationAllowance}</dd></div></dl>
  </article>;
}
...
function ProxyOrderForm({ product, isTrial }: { product: CatalogProduct; isTrial?: boolean }) {
  ...
  return <>
    <tr className="border-b border-[#edf2f3] last:border-0">
      ...
      <td className="px-4 py-4 text-xs font-extrabold text-[#13716e]"><div className="whitespace-nowrap">{unitCreditCost === null ? '—' : formatCredits(unitCreditCost)}<span className="ml-1 font-normal text-slate-400">{t('perDay')}</span></div>{unitIdrPrice !== null && unitCreditCost !== null && <p className="mt-1 max-w-[180px] whitespace-normal text-[10px] font-medium leading-4 text-slate-400">{formatUsd(product.unitPrice)} ≈ {formatIdr(unitIdrPrice)} ≈ {formatCredits(unitCreditCost)}</p>}</td>
