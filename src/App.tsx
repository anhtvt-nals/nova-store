...
function DashboardBillingPage() {
  ...
  const [amountIdr, setAmountIdr] = useState(1_000);
  ...
  const estimatedFeeIdr = Math.round(amountIdr * gatewayFeeRate) + 300;
  const estimatedNetAmountIdr = Math.max(0, amountIdr - estimatedFeeIdr);
  ...
  const updateDepositAmount = (rawValue: string) => {
    ...
    if (Number.isSafeInteger(parsed) && parsed >= 1_000) setAmountIdr(parsed);
  };
