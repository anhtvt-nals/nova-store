...
function useLocalePreferences() {
  ...
  return { locale, setLocale, t: (key: TranslationKey) => translations[locale][key], formatMoney, brandName: settings.data?.brandName || 'Nodenesia' };
}
...
function ClientHeader({ active = 'services' }: { active?: 'services' | 'proxy' | 'security' }) {
  ...
  const { t } = useLocalePreferences();
  return <header className="sticky top-0 z-40 border-b border-[#dbe7e9] bg-white/90 backdrop-blur-xl"><div className="mx-auto flex h-[72px] max-w-7xl items-center justify-between gap-4 px-5 lg:px-8"><Logo /><nav className="hidden items-center gap-1 md:flex"><Link href="/client" className={linkClass(active === 'services')}>{t('services')}</Link><Link href="/client/proxy" className={linkClass(active === 'proxy')}>SOCKS5 Proxy</Link><Link href="/client/security" className={linkClass(active === 'security')}>{t('security')}</Link></nav><div className="flex items-center gap-2"><LocaleSwitcher />{identity.data?.role === 'admin' && <Link href="/admin" className="hidden rounded-xl px-3 py-2 text-xs font-bold text-[#13716e] hover:bg-[#def5f3] sm:inline-flex">{t('admin')}</Link>}<div className="hidden text-right sm:block"><p className="text-xs font-bold">{displayName}</p><p className="text-[10px] text-slate-500">{user?.email}</p></div><button onClick={logout} className="grid h-10 w-10 place-items-center rounded-xl border border-[#dbe7e9] bg-white text-slate-600 hover:border-[#f46c43] hover:text-[#e05c37]" aria-label={t('signOut')}><LogOut size={16} /></button></div></div></header>;
}
