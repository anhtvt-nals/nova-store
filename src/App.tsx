...
function ProxyOrderForm({ product, isTrial }: { product: CatalogProduct; isTrial?: boolean }) {
  ...
  return <>
    <tr className="border-b border-[#edf2f3] last:border-0">
      ...
      <td className="whitespace-nowrap px-4 py-4 text-sm font-extrabold text-[#142037]">{quote.isLoading ? '…' : <>{formatMoney(quote.data?.total || 0, quote.data?.currency || product.currency)}<span className="mt-1 block text-[10px] font-medium text-[#13716e]">{quote.data?.creditCost?.toLocaleString() || 0} cr · {creditBalance.data?.balance?.toLocaleString() || 0} available</span></>}</td>
