...
function BillingInvoiceHistory() {
  const invoices = useSumopodInvoices();
  return <section className="mt-8"><SectionTitle eyebrow="payment history" title="Deposit invoices" body="Your 50 most recent Sumopod credit deposit invoices." /><State loading={invoices.isLoading} error={invoices.isError} onRetry={() => invoices.refetch()} empty={!invoices.data?.length}><div className="overflow-hidden rounded-3xl border border-[#dbe7e9] bg-white"><div className="divide-y divide-[#edf2f3]">{invoices.data?.map(invoice => <div key={invoice.id} className="grid gap-3 px-5 py-4 sm:grid-cols-[1fr_auto] sm:items-center"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><p className="text-sm font-extrabold text-[#142037]">{invoice.amountIdr.toLocaleString('id-ID')} IDR</p><Badge tone={invoice.status === 'completed' ? 'green' : invoice.status === 'pending' ? 'orange' : invoice.status === 'failed' ? 'red' : 'neutral'}>{invoice.status}</Badge></div><p className="mt-1 text-xs text-slate-500">Fee: {invoice.feeIdr === null ? '—' : `${invoice.feeIdr.toLocaleString('id-ID')} IDR`} · Credit: {invoice.creditAmount.toLocaleString(undefined, { maximumFractionDigits: 2 })} cr</p><p className="mono mt-1 text-[10px] text-slate-400">{invoice.id}</p></div><div className="text-left text-xs text-slate-400 sm:text-right"><p>{new Date(invoice.createdAt).toLocaleString()}</p>{invoice.completedAt && <p className="mt-1 text-[#13716e]">Completed {new Date(invoice.completedAt).toLocaleString()}</p>}</div></div>)}</div></div></State></section>;
}
