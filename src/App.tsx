...
import {
  ...
  useCreditBalance, useCurrentUser, useExtendOrder, useExportProxyConnections, useOrderQuote, useRecreateAllProxyNodes, useRestartProxyNode, useCreateStaticResidentialOrder, useExtendStaticResidentialOrder, useExportStaticResidentialConnections, useListStaticResidentialOrders, useStaticResidentialQuote, useImportStaticResidentialInventory, useStaticResidentialInventory, useStaticResidentialPricing, useUpdateStaticResidentialPricing,
...
import type {
  AdminOrder, AdminProduct, CatalogProduct, Category, CreditWallet, GeneralSettings, Plan, ProductInput, ProxyNode, ProxyProvider, RuntimeProxyNode, SandboxKey, User, Order, ConnectionDetails, StaticResidentialOrder,
} from '@/lib/api-client';
...
function AppShell({ children, admin = false }: { children: ReactNode; admin?: boolean }) {
  ...
  const nav = admin ? [
    ...
    { section: 'Proxy' }, { href: '/admin/proxy/api-keys', label: 'Provider API keys', icon: KeyRound }, { href: '/admin/proxy/providers', label: 'Providers', icon: Server }, { href: '/admin/proxy/orders', label: 'Orders', icon: Layers3 }, { href: '/admin/proxy/settings', label: 'Pricing', icon: Settings }, { href: '/admin/static-residential', label: 'Static residential', icon: Globe2 },
...
function AdminStaticResidentialPage() {
  const inventory = useStaticResidentialInventory();
  ...
  const submitImport = () => importer.mutate({ data: { content, label: label || undefined } }, { onSuccess: result => { setContent(''); window.alert(`Imported ${result.imported} proxy${result.imported === 1 ? '' : 'ies'}; skipped ${result.skipped}.`); void qc.invalidateQueries({ queryKey: getStaticResidentialInventoryQueryKey() }); }, onError: error => window.alert(error.message) });
  const available = inventory.data?.filter(item => item.status === 'available').length || 0;
  return <PageLayout admin eyebrow="static residential" title="US Static Residential Proxy" body="Import upstream SOCKS5 endpoints securely. Customer pages never receive this inventory or its credentials."><div className="grid gap-5 xl:grid-cols-[.8fr_1.2fr]"><div className="space-y-5"><div className="rounded-3xl border border-[#dbe7e9] bg-white p-6"><h2 className="font-extrabold">Fixed package pricing</h2><p className="mt-2 text-sm leading-6 text-slate-500">Every order has exactly 5 ports and one shared 5GB quota. Price is per GB/day, so checkout is 5 × days × this rate.</p><label className="mt-5 block text-sm font-bold">USD per GB / day<input className="mt-2 w-full rounded-xl border border-[#dbe7e9] bg-[#f8fbfb] px-3 py-3 text-sm" inputMode="decimal" value={price} onChange={event => setPrice(event.target.value)} /></label><Button className="mt-4" disabled={updatePrice.isPending || !(Number(price) > 0)} onClick={savePrice}>{updatePrice.isPending ? 'Saving…' : 'Save price'}</Button></div><div className="rounded-3xl border border-[#dbe7e9] bg-white p-6"><h2 className="font-extrabold">Import upstream proxies</h2><p className="mt-2 text-sm leading-6 text-slate-500">One SOCKS5 URL per line. Passwords are encrypted at rest and are never returned by this page.</p><input className="mt-4 w-full rounded-xl border border-[#dbe7e9] bg-[#f8fbfb] px-3 py-3 text-sm" placeholder="Optional batch label" value={label} onChange={event => setLabel(event.target.value)} /><textarea className="mt-3 min-h-48 w-full rounded-xl border border-[#dbe7e9] bg-[#f8fbfb] px-3 py-3 font-mono text-xs outline-none focus:border-[#f46c43]" placeholder={'socks5://user:pass@host:port\nsocks5://user:pass@host:port'} value={content} onChange={event => setContent(event.target.value)} /><Button className="mt-3 w-full" disabled={!content.trim() || importer.isPending} onClick={submitImport}>{importer.isPending ? 'Importing…' : 'Import TXT list'}</Button></div></div><div className="rounded-3xl border border-[#dbe7e9] bg-white"><div className="flex items-center justify-between border-b border-[#edf2f3] px-5 py-4"><div><h2 className="font-extrabold">Inventory</h2><p className="mt-1 text-xs text-slate-500">{available} available · 5 required for every new order</p></div><Badge tone={available >= 5 ? 'green' : 'red'}>{available >= 5 ? 'ready' : 'low capacity'}</Badge></div><State loading={inventory.isLoading} error={inventory.isError} onRetry={() => inventory.refetch()} empty={!inventory.data?.length}><div className="divide-y divide-[#edf2f3]">{inventory.data?.map(item => <div key={item.id} className="flex items-center justify-between gap-4 px-5 py-3"><div className="min-w-0"><p className="truncate text-sm font-bold">{item.label || 'Imported proxy'} · #{item.id}</p><p className="mono mt-1 truncate text-[10px] text-slate-500">{item.host}:{item.port} · {item.username}</p></div><Badge tone={item.status === 'available' ? 'green' : item.status === 'assigned' ? 'orange' : 'red'}>{item.status}</Badge></div>)}</div></State></div></div></PageLayout>;
}
...
function ClientDockNav({ active }: { active: 'services' | 'proxy' | 'static' }) {
  ...
  return <nav aria-label="Client navigation" className="group fixed right-3 top-1/2 z-30 -translate-y-1/2 overflow-hidden rounded-2xl border border-[#dbe7e9] bg-white/95 p-1.5 shadow-[0_12px_35px_rgba(20,32,55,.14)] backdrop-blur-xl md:right-5 md:w-12 md:transition-[width] md:duration-300 md:hover:w-48">
    <div className="grid gap-1">{items.map(item => <Link key={item.href} href={item.href} title={item.label} aria-current={item.active ? 'page' : undefined} className={cx('flex h-10 items-center gap-3 overflow-hidden rounded-xl px-2.5 text-sm font-bold transition', item.active ? 'bg-[#eaf8f6] text-[#13716e]' : 'text-slate-500 hover:bg-[#f4f8f8] hover:text-[#142037]')}><item.icon size={17} className="shrink-0" /><span className="whitespace-nowrap opacity-0 transition-opacity duration-150 md:group-hover:opacity-100">{item.label}</span></Link>)}</div>
  </nav>;
}
...
function ClientDashboardPage() {
  ...
  const services = [
    ...
    { name: 'US Static Residential Proxy', description: 'Five fixed SOCKS5 ports with 5GB shared traffic and hourly upstream rotation.', status: 'available', href: '/client/static-residential', route: '/client/static-residential', icon: Globe2, tone: 'orange' },
...
type ProtectedPage = 'client-dashboard' | 'client-proxy' | 'client-static-residential' | 'admin-security' | 'admin-overview' | 'admin-catalog' | 'admin-info-users' | 'admin-credits' | 'admin-proxy-api-keys' | 'admin-proxy-providers' | 'admin-proxy-orders' | 'admin-proxy-settings' | 'admin-static-residential' | 'admin-settings';
...
function RouterViews() {
  return <RoutedErrorBoundary><Switch><Route path="/" component={HomeRedirect} /><Route path="/sign-in/*?" component={AuthPage} /><Route path="/sign-up/*?" component={() => <Redirect to="/sign-in" />} /><Route path="/client/nodes" component={() => <Redirect to="/client/proxy#my-services" />} /><Route path="/client/orders" component={() => <Redirect to="/client/proxy#orders" />} /><Route path="/client/proxy" component={() => <Protected page="client-proxy" />} /><Route path="/client/static-residential" component={() => <Protected page="client-static-residential" />} /><Route path="/client" component={() => <Protected page="client-dashboard" />} /><Route path="/admin/security" component={() => <Protected page="admin-security" />} /><Route path="/admin/users" component={() => <Redirect to="/admin/info/users" />} /><Route path="/admin/credits" component={() => <Protected page="admin-credits" />} /><Route path="/admin/keys" component={() => <Redirect to="/admin/proxy/api-keys" />} /><Route path="/admin/orders" component={() => <Redirect to="/admin/proxy/orders" />} /><Route path="/admin/info/users" component={() => <Protected page="admin-info-users" />} /><Route path="/admin/proxy/api-keys" component={() => <Protected page="admin-proxy-api-keys" />} /><Route path="/admin/proxy/providers" component={() => <Protected page="admin-proxy-providers" />} /><Route path="/admin/proxy/orders" component={() => <Protected page="admin-proxy-orders" />} /><Route path="/admin/proxy/settings" component={() => <Protected page="admin-proxy-settings" />} /><Route path="/admin/static-residential" component={() => <Protected page="admin-static-residential" />} /><Route path="/admin/settings" component={() => <Protected page="admin-settings" />} /><Route path="/admin/catalog" component={() => <Protected page="admin-catalog" />} /><Route path="/admin" component={() => <Protected page="admin-overview" />} /><Route component={NotFound} /></Switch></RoutedErrorBoundary>;
}
