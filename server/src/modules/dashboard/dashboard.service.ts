...
export class DashboardService {
  ...
  async clientOverview(user: AuthUser) {
    ...
    const [orderResult, usageResult, nodeResult] = await Promise.all([
      ...
      this.db.client.from('usage_daily').select('requests,successful_requests').eq('profile_id', user.profileId).eq('usage_date', now.slice(0, 10)),
    ...
    const usage = this.db.unwrap(usageResult, 'Unable to load usage') as Array<{ requests: number; successful_requests: number }>;
    if (nodeResult.error) throw nodeResult.error;
    const requests = usage.reduce((sum, item) => sum + Number(item.requests), 0);
    const successful = usage.reduce((sum, item) => sum + Number(item.successful_requests), 0);
